...
"""Strict same-Reference GPQA A/B: current route logic vs v0.6 unified route.

The initial Reference tuple is retrieved once per case.  The current-route HDS
supervisor is intentionally given no external reference provider, so neither arm
can enlarge R during the comparison.  Gold is used only after both outputs.
Sandbox branch only.
"""
...
def _current_same_r(question_ir, references, *, compiler):
    initial = HDS選択推論実行(
        question_ir,
        tuple(references),
    ...
    if tuple(supervised.参照) != tuple(references):
        raise RuntimeError("strict same-Reference invariant violated by current arm")
    return supervised.選択


def _v06_same_r(question_ir, references, *, compiler):
    ...
    if tuple(session.参照正本) != tuple(references):
        raise RuntimeError("strict same-Reference invariant violated by v0.6 arm")
...
def main() -> int:
    ...
    out = Path(os.environ.get("MINIDORA_V06_AB_OUT", f"v06_strict_ab_{start:03d}-{end - 1:03d}.json"))
    if not (0 <= start < end <= 198):
        raise ValueError(f"invalid range {start}:{end}")

    with tempfile.TemporaryDirectory(prefix="minidora-v06-strict-ab-") as td:
        ...
        initial_docs = retrieval_empty = 0
        ...
        for index, (question, choices, gold) in enumerate(cases, start=start):
            ...
            current = _current_same_r(question_ir, references, compiler=compiler)
            v06, snapshot = _v06_same_r(question_ir, references, compiler=compiler)
            ...
            print(
                f"CASE {index + 1:03d}/198 R={len(references)} current={current.状態}:{c_pred} "
        ...
        payload = {
            "schema": "minidora.gpqa.v06-strict-same-reference-ab.segment.v1",
            "protocol": {
                ...
                "reference_rule": "one initial Reference tuple per case; no later external Reference acquisition in either arm",
                "current_arm": "current formal choice + current supervisor with reference provider disabled",
            ...
            "current_same_reference_control": current_metric,
            ...
            "retrieval": {
                "reference_documents": initial_docs,
                "retrieval_empty_cases": retrieval_empty,
            },
            "details": details,
        }
        out.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        print("STRICT_SEGMENT_RESULT=" + json.dumps({"range": [start, end], "current": current_metric, "v06": v06_metric, "delta": payload["delta"]}, ensure_ascii=False), flush=True)
        return 0
