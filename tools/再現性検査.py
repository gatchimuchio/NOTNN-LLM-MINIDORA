...
def main():
    ...
    if len(sys.argv)==2 and sys.argv[1]=='--子実行':
        ...
        結果=[asdict(改善計画を実行(r['種類'],r['要求'],詳細=False)) for r in rows]
        print(sha256(_符号化(結果)).hexdigest());return 0
    outputs={}
    for seed in ('0','1','913'):
