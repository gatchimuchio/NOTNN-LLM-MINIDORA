...
def 実測(repeats=4):
    ...
    rows=[]
    for name,(plan,資料) in workloads.items():
        参照=None
        for enabled in (False,True):
            session=統合セッション('調整対照',再利用=enabled)
            durations=[];counts=[];hashes=[]
            for _ in range(repeats):
                start=perf_counter_ns();r=session.計画実行(plan,資料);durations.append((perf_counter_ns()-start)/1000000)
                if not r.成立:
                    raise ValueError(f'{name}:{r.理由}')
                values=_符号化([(k,_結果辞書(v)) for k,v in r.出力])
                if 参照 is None:参照=values
                if 参照!=values:raise ValueError('ON/OFF・反復の出力不一致')
    ...
    情報源=能力結果(True,'。'.join(f'項目{i}' for i in range(20))+'。')
    p=合成計画((合成工程('変換',('文脈変換',),'指示',(素材参照('入力','本文'),),'設定'),),('変換',))
    d={'指示':能力結果(True,'箇条書き化'),'本文':情報源,'設定':能力結果(True,'',データ={'形式':'箇条書き'})}
