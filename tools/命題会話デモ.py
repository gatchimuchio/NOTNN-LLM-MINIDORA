...
def 実演():
    ...
    rows = []
    for text, 状態, expected in script:
        結果 = session.応答(text)
        decision = None
        if 結果.結果:
            decision = 結果.結果.データ['元結果'][0]['データ']['判定結果']['判定']
        if 結果.状態 != 状態 or decision != expected:
            raise RuntimeError('会話接続が期待と不一致:' + text + ':' + 結果.理由)
        rows.append({'入力': text, '状態': 結果.状態, '判定': decision, '本文': 結果.本文,
                     '意味候補数': len((結果.追跡 or {}).get('命題解釈候補', ()))})
