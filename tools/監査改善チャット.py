...
def main(argv=None):
    ...
    if args.contract:
        ...
        print(_符号化({
            '版': 'MINIDORA-会話CLI-v0.3', '意味契約': 契約版,
            ...
            '例': ['命題資料「例」を登録：P。PならばQ。', '資料「例」から「Q」を判定して', '短く説明して',
                  '資料「例」に基づいて「Q」を判断してくれる？、短く説明して',
                  '命題資料「分類」を登録：別名：述語「別表記」=「正規名」\n対象は別表記である。'],
            '対応範囲': '登録資料上の有限命題・候補仮説・明示ブール介入。任意自然言語や一般常識の理解を意味しない',
            '不正通信': '終了値2。出力ファイルと保存状態を公開しない。標準出力は巻戻しできない',
            '意味拡張': '依頼語尾と表示指定を有限に分離。別名は資料の明示定義だけを使い、発言・信念内を置換しない',
            '探索': '既定は観測の関連閉包。全列挙での比較も明示できる。候補の自動生成・尤度評価はしない',
            '保存互換': '意味契約が異なる過去版は自動移行しない。過去版と原記録を保持する',
