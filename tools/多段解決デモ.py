...
def 問題を用意(値: int = 120, 代替あり: bool = True):
    ...
    資料 = {"原資料": 能力結果(True, text, 参照=(参照資料("元", "人工資料", "局所デモ", 本文=text),)),
    ...
    if 代替あり:
        methods.append(解法("原文を保持", "素材準備", (), "素材引継ぎ", "指示",
                             (問題素材("入力", "原資料"),), 優先度=1))
    return 多段問題(("全数値抽出",), goals, tuple(methods)), 資料
...
def main() -> int:
    ...
    p, 資料 = 問題を用意(args.値, not args.代替なし)
    結果 = 多段解決器((*局所能力群(), *解決補助能力群()), 純粋作用確認=True).実行(p, 資料)
    print(json.dumps({"範囲": "宣言した有限解法からの目的分解・後戻りの局所試験。自由文理解ではない。",
        "状態": 結果.状態, "理由": 結果.理由, "出力": {k: v.本文 for k, v in 結果.出力},
        "採用解法": [r["解法"] for r in 結果.採用経路],
        "検証で退けた目的": [r["目的"] for r in 結果.履歴 if r["作用"] == "目的条件未達"],
        "展開数": 結果.展開数, "能力呼出数": 結果.呼出数, "監査整合": 結果.整合確認()},
        ensure_ascii=False, indent=2))
    expected = "保留" if args.代替なし else "合格"
    return 0 if 結果.状態 == expected and 結果.整合確認() else 1
