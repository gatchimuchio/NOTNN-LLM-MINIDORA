...
def main() -> int:
    ...
    parser.add_argument("--留保",作用="store_true")
    ...
    資料={"資料":能力結果(True,"",参照=refs),"比較指示":能力結果(True,"数値主張を比較"),
    ...
    結果=能力合成器((証拠統合モジュール().登録(),応答構成モジュール().登録())).実行(plan,資料)
    answer=結果.出力[0][1] if 結果.成立 else None
    print(json.dumps({"範囲":"人工資料の数値比較からの回答構成。自由作文・LIVE評価ではない。",
                      "状態":結果.状態,"理由":結果.理由,
                      "回答":answer.本文 if answer else "",
                      "項目状態":answer.データ["項目状態"] if answer else [],
                      "実行能力":[x.能力 for x in 結果.履歴],
                      "合成監査":結果.監査整合(),"回答監査":応答記録整合(answer) if answer else False},ensure_ascii=False,indent=2))
