...
def 意味被覆監査(cases, compiler) -> dict[str, object]:
    ...
    for index, (question, choices) in enumerate(cases):
        ...
        question_no_relation_no_semantic_loss += int(not q_relations and not _意味損失(question_ir))
        question_relation_types.update(_関係種別群(question_ir))
        question_k_relation_types.update(_関係種別群(k_question))
        ...
        for choice in choices:
            ...
            candidate_no_relation_no_semantic_loss += int(not relation_present and not _意味損失(candidate_ir))
        ...
        details.append(
            {
                ...
                "question_semantic_loss": _意味損失(question_ir),
