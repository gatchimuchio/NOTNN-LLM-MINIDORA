...
def main() -> int:
    ...
    parser.add_argument("--競合", 作用="store_true")
    ...
    runner = 能力合成器((証拠統合モジュール().登録(), 記載値採用モジュール().登録(), *局所能力群()))
    結果 = runner.実行(plan, 資料)
    proof = dict(結果.中間結果).get("証拠")
    print(json.dumps({"範囲":"人工資料の局所対照。事実性の認定ではない。",
                      "入力":[first,second], "状態":結果.状態, "理由":結果.理由,
                      "最終出力":[v.本文 for _,v in 結果.出力],
                      "実行能力":[t.能力 for t in 結果.履歴],
                      "証拠報告":proof.データ if proof else None,
                      "監査整合":結果.監査整合()}, ensure_ascii=False, indent=2))
    correct = (not 結果.成立 and 結果.実行数 == 2 if args.競合 else
               結果.成立 and 結果.出力[0][1].本文 == str(args.値))
    return 0 if correct and 結果.監査整合() else 1
