...
def 用意(値: int = 120, 列名: str = '金額'):
    text=f'番号,品目,金額,備考\r\n001,甲,{値},採用候補\r\n002,乙,999,別件\r\n'
    資料={'原資料':能力結果(True,text,参照=(参照資料('CSV原典','人工明細','局所試験',本文=text),)),
...
def main():
    ...
    結果=能力合成器((*構造化文書能力群(),*局所能力群())).実行(plan,資料)
    documents=[v for _,v in 結果.中間結果 if v.データ.get('版')=='MINIDORA-構造化文書-v0.1']
    final=結果.出力[0][1].本文 if 結果.成立 else ''
    selected=dict(結果.中間結果).get('値本文')
    ok=(not 結果.成立 and not 結果.出力) if args.欠落列 else 結果.成立 and final==str(args.値)
    print(json.dumps({'範囲':'明示行列からの文書処理。意味的に正しい選択かは上位の責任。',
        '成立':結果.成立,'理由':結果.理由,'最終結果':final,
        '中間本文':{k:v.本文 for k,v in 結果.中間結果},
        '元セル対応':selected.データ['文書']['対応'].get('') if selected else None,
        '未選択位置':selected.データ['未選択位置'] if selected else [],
        '実行数':結果.実行数,'合成監査':結果.監査整合(),
