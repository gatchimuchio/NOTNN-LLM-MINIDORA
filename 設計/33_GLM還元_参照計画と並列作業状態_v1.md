...
状態: **実験・構文化資産（active 模型核ではない）**  
旧本文保存先: `実験/33_GLM還元_参照計画と並列作業状態_v1.md`

この文書が以前「現行標準経路」と記述していたV3 / adaptive arbitration / J経路は、2026-09-05の独立監査で `32_MINIDORA_HDS監督介入制御_v1.md` の責任境界と衝突することが確認されたため、active 模型核から隔離した。

GLM由来の構文化成果自体は破棄しない。次の条件を満たす一般作用だけを模型核へ個別還元する。
