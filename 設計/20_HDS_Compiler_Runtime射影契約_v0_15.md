# HDS 構文化器 実行系射影契約 v0.15

## 1. 目的

HDS 構文化器は、自然言語から得た情報を一つの平坦な語集合としてミニドラへ渡す装置ではない。

構文化器は入力に含まれる意味を可能な限り保持した完全HDS-IRを作り、実行系はその完全IRから各責務が実際に消費できる最小十分なProjectionだけを渡す。
...
### 2.1 構文化器が解釈し、実行系は射影する

自然言語の構文、語形、否定、様相、条件、関係方向、未知端点の解釈は構文化器責務である。

実行系 Projectionは自然言語を再解析しない。
...
基礎構文化器の関係抽出が端点へ吸収した助動・否定は、`HDS英語関係範囲射影`が構文化器内部で実体から分離し、relation条件へ移す。
...
### 4.3 範囲層は新しい世界関係を作らない

範囲層は既存の構文化器関係を正規化・注釈する。

世界知識や共起から新規関係を推測しない。

疑問文の意味構造は質問意味構文化器へ委ねる。

---

## 5. R質問射影

Rは「問題を理解する主体」ではない。構文化器が確定した検索要求を外部参照へ運ぶ境界である。
...
検索器が代替経路時に元の完全質問文を読み直して制御語を再混入させてはならない。
...
`目的.検索焦点`が主題語抽出を抑制しないよう、構文化器完成工程で未所有の焦点語を`対象.主題語`へ別役割として射影する。
...
- `表層.*`
- query route
- Kでrelationへ束縛できない範囲座標

### 8.1 未対応範囲を肯定Factへ潰さない

現在のKが否定・様相・条件範囲を関係演算として表現できない場合、
...
を保持し、実行系 K射影はその構造だけを見て有向辺への昇格を止める。
...
1. 完全HDS-IRを破壊しない。
2. 自然言語の意味解釈を実行系へ移さない。
3. 実行系 Projectionは構造選択・loweringだけを行う。
4. Rへ選択制御・監査メタを検索対象として渡さない。
5. Kへ検索・制御・目的・監査・表層メタを世界Factとして渡さない。
6. 未知端点と既知関係種別を同じ未観測状態へ潰さない。
7. Kで表現できない否定・様相・条件範囲を無条件肯定辺へ潰さない。
8. 範囲分離前の汚染端点を意味対象として再利用しない。
9. 片端しか残らない関係をKへ作らない。
