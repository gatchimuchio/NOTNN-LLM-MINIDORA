...
汚染された元端点は消去しない。完全IR上で`表層.端点原形`へ降格して観測表層として保持する。
...
これにより監査可能性を保持しつつ、`A may`が実体としてK/Rへ再流入することを防ぐ。
...
`目的.検索焦点`が主題語抽出を抑制しないよう、Compiler完成工程で未所有の焦点語を`対象.主題語`へ別役割として射影する。
...
5. Kへ検索・制御・目的・監査・表層メタを世界Factとして渡さない。
6. 未知端点と既知関係種別を同じ未観測状態へ潰さない。
7. Kで表現できない否定・様相・条件scopeを無条件肯定辺へ潰さない。
8. scope分離前の汚染端点を意味対象として再利用しない。
9. 片端しか残らない関係をKへ作らない。
10. query routeを世界証拠へ昇格させない。
11. J/Mは完全IRを保持する。
12. benchmark固有分岐を入れない。
13. 世界知識をCompilerへ追加しない。
