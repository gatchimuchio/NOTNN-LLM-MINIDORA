...
現行の格役割・条件方向・明示例外と旧保存移行は[全体運用v6](docs/HDS全体運用_v6_利用方法.md)を参照してください。
[全体運用v5](docs/HDS全体運用_v5_利用方法.md)の契約と旧記録も保持しています。
...
## HDS全体運用v4：資料意味・内容構成・明示移行

資料の主題・見出し・明示別名から原文を選択し、条件・例外を同伴して内容仕様を構成する。既存文章作成を同じHDSの工程へ接続し、根拠付きの資料説明、観点別記載対照、追加確認事項、再表現を扱う。既存の公開本文取得とも接続した。標準v1〜v3保存は原本・履歴・依存を保持して明示移行できる。

[利用方法](docs/HDS全体運用_v4_利用方法.md) / [設計66](設計/66_HDS_MINIDORA_資料意味と内容構成_v4.md) / [全開発順](docs/HDS全能力開発_ロードマップ.md) / [台帳](docs/HDS全能力開発_台帳.json)

これは一般依頼への能力拡張の最初の増分であり、任意自然文・自由作文・翻訳・未見解法の全完成ではない。旧版の実装報告は当時の記録として保持する。


## HDS全体運用v5：関係・条件・不足から説明する

既存HDSと命題推論・知識取得・文章作成を利用し、普通の名詞述語・サ変活用・明示条件・限定照応から実導出と未確認前提を構成する。取得は実本文を再読解し、取得成功だけで結論を補わない。保存版v5と標準v1〜v4の原本・移行履歴保持に対応する。

[利用方法](docs/HDS全体運用_v5_利用方法.md) / [設計67](設計/67_HDS_MINIDORA_関係読解と根拠説明_v5.md) / [実装検証](docs/HDS全体運用_v5_実装検証.md)

順序2の一部を進めた増分であり、任意文章理解、一般的な資料間同一性、全知識の供給、自由作文、未見課題の一般解法まで完成したものではない。性能正本値は更新しない。
