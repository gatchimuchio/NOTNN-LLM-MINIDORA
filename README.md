...
統合運用の互換CLI入口は維持しています。

```bash
python -m minidora.製品版 --HDS --serve
```

公開READMEでは、内部理論の抽象定義、上位参照先、完全な責任分解は説明しません。
利用可能な外部挙動はCLI・API・試験・評価結果で確認してください。
...
|---|---|---:|---|
| 模型核 | **MINIDORA30** | **30 / 198 (15.15%)** | 現行模型核汎用E2E性能セーブポイント |
...
統合運用の各版は実装・試験・評価記録として保持しますが、公開READMEでは内部理論の分解や工程対応を再掲しません。
個別の公開仕様は、入出力・制限・検証条件だけを参照してください。
