...
from .hds_language_auxiliaries import HDS英語補助語端点正規化
...
class 公開HDSコンパイラ(_基礎HDSコンパイラ):
    ...
    def _完成(self, base: HDSIR, *, HDS履歴: tuple[HDSIR, ...] = ()) -> HDSCompiler成果:
        ...
        # 時制・相の補助語が関係始点へ吸収された場合だけ実体端点と分離する。
        # modal/否定はこの層ではactualへ変換しない。
        base = HDS英語補助語端点正規化(base)
        ...
        return replace(refreshed, IR=final_ir, 状態遷移=graph, 暗黙知構造=tacit, 失敗署名候補=signatures, チェックリスト=checklist, 認知世界差分=world_diff, 監査参照候補=audit_queries)
