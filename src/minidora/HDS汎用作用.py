...
class HDS参照取得作用:
    ...
    def 機会(self, 状態: HDS実行状態) -> HDS作用機会 | None:
        ...
        return HDS作用機会(
            self.作用ID,
            f"{状態.状態署名}:{問合せ}:{self.取得上限}:{getattr(self.供給器, '名称', '')}",
            self.入力状態,
            frozenset({self.出力状態}),
            self.解消対象,
            self.資源負荷,
            0.0,
            True,
            ("REFERENCE_PROVIDER_AS_HDS_OBSERVATION",),
        )
    ...
    def 実行(self, 状態: HDS実行状態) -> HDS作用結果:
        ...
        return HDS作用結果(
            ...
            解消残差=self.解消対象,
            成果=(("参照記録", 記録群),),
            理由=("REFERENCE_FETCHED", f"件数:{len(記録群)}"),
        )
...
class HDS計算実行作用:
    ...
    def 機会(self, 状態: HDS実行状態) -> HDS作用機会 | None:
        ...
        return HDS作用機会(
            self.作用ID,
            f"{状態.状態署名}:{repr(中間表現)}:{repr(sorted(初期状態.items(), key=lambda 行: str(行[0])))}",
            self.入力状態,
            frozenset({self.出力状態}),
            self.解消対象,
            self.資源負荷,
