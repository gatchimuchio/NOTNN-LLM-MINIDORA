...
_受動未知対象 = re.compile(
    ...
    rf"(?:is|are|was|were|has\s+been|have\s+been|had\s+been)\s+{_関係句}\s+by\s+(?P<s>.+)$",
    re.I,
)
_能動未知対象 = re.compile(
    rf"^(?:which|what)\s+(?:of\s+the\s+following\s+)?{_型}\s+"
...
_制御規則 = (
    ...
    ("様相", re.compile(r"\b(?:can|could|may|might)\b", re.I), "可能"),
...
_検索除外 = frozenset(
    {
        ...
        "had", "most", "least", "likely", "unlikely", "following", "statement", "statements", "answer", "answers",
        "option", "options", "choice", "choices", "correct", "incorrect", "true", "false", "best", "select", "choose",
    }
)
...
def 英日意味フレーム抽出(text: str) -> 英日意味フレーム:
    ...
    return 英日意味フレーム(tuple(canonical), _検索語(raw), controls, question)
