'既存の抽出要約・情報抽出・文脈変換を、合成器へ接続する薄い適合器。\n\n自由文の要求解釈は行わない。工程設定は計画とは別の初期資料から受け取る。\n'
from __future__ import annotations

from .能力合成 import 登録能力
from .製品版.型 import 能力結果
from .製品版.能力契約 import 能力文脈
from .製品版.要約 import 汎用要約モジュール
from .製品版.抽出 import 情報抽出モジュール
from .製品版.変換 import 文脈変換モジュール


class 抽出要約接続:
    名前 = "抽出要約"
    版 = "合成要約接続-v0.1/" + 汎用要約モジュール.版
    優先度 = 0
    ...
    def 実行(self, 文脈: 能力文脈) -> 能力結果:
        ...
        return 汎用要約モジュール().実行(文脈.直前応答, 行数=行数, 参照=文脈.直前参照)


class 情報抽出接続:
    名前 = "情報抽出"
    版 = "合成抽出接続-v0.1/" + 情報抽出モジュール.版
    優先度 = 0
    ...
    def 実行(self, 文脈: 能力文脈) -> 能力結果:
        ...
        return 情報抽出モジュール().実行(種別, 文脈.直前応答)


class 文脈変換接続:
    名前 = "文脈変換"
    版 = "合成変換接続-v0.1/" + 文脈変換モジュール.版
    優先度 = 0
    ...
    def 実行(self, 文脈: 能力文脈) -> 能力結果:
        ...
        return 文脈変換モジュール().実行(形式, 文脈.直前応答)
