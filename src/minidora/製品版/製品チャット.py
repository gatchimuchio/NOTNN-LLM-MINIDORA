...
def _不採用本文(result: 能力結果) -> str:
    reason = result.保留理由 or result.状態.value
    if result.状態 == 実行状態.失敗:
        return f"処理に失敗しました。{reason}"
    if result.状態 == 実行状態.非適用:
...
class 製品ミニドラ:
    ...
    def _模型核(self,text:str)->tuple[能力結果,dict[str,Any]]:
        ...
        if hasattr(self.基礎ミニドラ,"実行"):
            try:
                ...
                return 能力結果(False,"",保留理由=f"Core実行失敗:{type(exc).__name__}",採否状態=実行状態.失敗),trace
            value=getattr(r,"値",None); refs=getattr(r,"参照",()) or (); hist=getattr(r,"履歴",()) or ()
            decision=getattr(r,"採否",None); state=getattr(decision,"状態",None); reasons=tuple(getattr(decision,"理由",()) or ())
            status=getattr(state,"value",str(state or ""))
            trace={"値":value,"採否":status,"採否理由":reasons,"参照数":len(refs),"履歴":hist,"言語計画":getattr(r,"言語計画",None),"HDS_IR":repr(getattr(r,"HDS_IR",None))}
            if not isinstance(state,実行状態):
                return 能力結果(False,"",保留理由="Core採否状態不正",採否状態=実行状態.失敗,データ=trace),trace
            if state != 実行状態.合格:
                reason=" / ".join(str(x) for x in reasons) or f"Core状態:{state.value}"
                return 能力結果(False,"" if value is None else str(value),根拠=("MINIDORA Core採否",),データ=trace,保留理由=reason,採否状態=state),trace
            if value is None:
                return 能力結果(False,"",保留理由="Core合格結果に値がない",採否状態=実行状態.失敗,データ=trace),trace
            return 能力結果(True,str(value),根拠=("MINIDORA Core実行結果",),データ=trace,採否状態=実行状態.合格),trace
        if hasattr(self.基礎ミニドラ,"応答"):
            try:
                response=str(self.基礎ミニドラ.応答(text)); trace={"追跡範囲":"公開応答境界"}
                return 能力結果(True,response,根拠=("MINIDORA Core応答",),データ=trace),trace
            except Exception as exc:
                trace={"例外":type(exc).__name__}
                return 能力結果(False,"",保留理由=f"Core応答失敗:{type(exc).__name__}",採否状態=実行状態.失敗),trace
        return 能力結果(False,"",保留理由="Core実行契約なし",採否状態=実行状態.非適用),{}
    ...
    def _応答_locked(self,text,st)->製品応答:
        ...
        if selected is None:
            ...
            except Exception as exc: result=能力結果(False,"",保留理由=f"Module実行失敗:{type(exc).__name__}",採否状態=実行状態.失敗)
        ...
        audit.記録("応答構成","製品チャット",製品チャット版,{"能力結果":result.成立,"結果状態":status},{"本文":body,"状態":status})
        ...
        return 製品応答(st.セッションID,body,status,route,record.追跡ID,record.ルートハッシュ,result.参照,(route,),{"capability_candidates":candidates,"result_state":status})

    def _改善対象(self,text,st):
        """既存Module経路を残し、明示された資料操作と同じ会話の継続だけを接続する。"""
        if not self._改善有効:
            return False
        with self._汎用ロック:
