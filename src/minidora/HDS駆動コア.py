...
class HDS駆動コア:
    ...
    def 選択実行(self, 問合せ: str, 選択肢: Sequence[str], *, 初期参照=(), 参照供給器=None,
        ...
        from .HDS選択継承循環 import (
            ...
            計算済み成果名, 選択閉包状態, 残差_未評価,
        )
        入力残差非阻害対象 = tuple(
            f"HDS残差:{項目.種別}:{項目.理由}" for 項目 in 入力束.残差 if 項目.種別 == "未解共参照"
        )
        供給 = HDS選択継承供給(
            問題束, self.HDSコンパイラ, tuple(初期参照), 模型核=模型核, 基礎能力核=基礎能力核,
            既存能力継承=既存能力継承, 参照供給器=参照供給器, 計算実行器_=計算実行器_,
            設定=HDS選択継承設定(最大回復回数), 拡張採用証明=拡張採用証明,
            入力残差非阻害対象=入力残差非阻害対象,
        )
        return self.実行(
            問合せ, 目的=("選択問題を閉包する",), 要求状態=(選択閉包状態,), 初期残差=(残差_未評価,),
            初期成果={参照成果名: tuple(初期参照), 参照世代成果名: 0, 計算済み成果名: False},
            カーネル正本=問題束,
            追加作用供給器=(HDS作用供給器("HDS選択継承循環", 供給.構成, "v3"),),
        )
