...
def _既定評価実行(
    ...
    model_core = getattr(環境, "能力模型核", None)
    return HDS選択推論実行(
        ir,
        references,
        コンパイル=環境.コンパイル,
...
def HDS駆動選択実行(
    ...
    *,
    ...
    """HDS Judgement Subjectが次作用を選びながら一回の選択問題を閉じる。

    参照・候補計算は計算主体側のworkerであり、worker自身は最終採用できない。
    COMMIT/SUSPENDは ``MINIDORAHDS判断主体`` だけが確定する。
    """
    ...
    while True:
        ...
        if request.作用 == HDS作用種別.確定:
            if selected is None:
                raise RuntimeError("HDS COMMIT要求に候補計算結果が存在しない")
        ...
        if request.作用 == HDS作用種別.停止:
            if world.状態 == "COMMITTED" and selected is not None:
                return HDS駆動選択結果("APPROVE", selected.回答内容, tuple(selected.理由), references, selected, world)
            if selected is None:
                selected = _保留選択(request.理由)
