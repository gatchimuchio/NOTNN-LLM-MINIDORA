...
def _compile_patterns() -> tuple[tuple[str, re.Pattern[str], re.Pattern[str]], ...]:
    rows: list[tuple[str, re.Pattern[str], re.Pattern[str]]] = []
    for kind, phrase in _比較句:
        ...
        rows.append((kind, declarative, unknown_start))
    return tuple(rows)
    ...
    for index, sentence in enumerate(sentences):
        ...
        if is_question:
            ...
            if match:
                ...
                out.append(英語比較意味("等価", requested, _normalize(match.group("o")), "equal to", "始点", requested))
                continue
            match = _等価動詞未知終点.fullmatch(sentence)
            if match:
                out.append(英語比較意味("等価", _normalize(match.group("s")), "未特定", "equal to", "終点", "未特定"))
                continue
        else:
        ...
        for kind, declarative, unknown_start in _比較規則:
            if is_question:
                match = unknown_start.fullmatch(sentence)
                if match:
                    requested = _requested_type(match.groupdict().get("kind") or "")
                    item = 英語比較意味(kind, requested, _normalize(match.group("o")), _normalize(match.group("v")).casefold(), "始点", requested)
                    if item not in out:
                        out.append(item)
            else:
                match = declarative.fullmatch(sentence)
                if match:
                    item = 英語比較意味(kind, _normalize(match.group("s")), _normalize(match.group("o")), _normalize(match.group("v")).casefold())
                    if item not in out:
                        out.append(item)
    return tuple(out)
