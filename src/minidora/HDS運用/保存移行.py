"""実測したv1〜v6保存契約からの明示移行。旧原本・履歴・依存を保持する。"""
...
移行版 = "HDS旧保存明示移行-v3"
_既知移行版 = {"HDS旧保存明示移行-v1", "HDS旧保存明示移行-v2", 移行版}
# 由来: tests/資料/HDS旧保存/由来.json。各固定commitの実保存と照合済み。
_旧契約 = {
    "HDS-MINIDORA-全体運用-v6": ("22f7d3d8783a0f191c2fbd74317cc12260fe9586d9b1b31be8c22966aa267c73", "574f22937b093985486d23c30c829d2b25cf7182"),
...
def _旧保存を読む(原本, *, 深さ=0):
    ...
    if type(状態) is not dict or 状態.get("版") not in _旧契約:
        raise ValueError("対応する旧保存版はv1・v2・v3・v4・v5・v6です")
    追加 = set() if 状態["版"].endswith("-v1") else _v2欄
    if 状態["版"].endswith(("-v4", "-v5", "-v6")):
        追加 = 追加 | {"移行履歴"}
    if set(状態) != _共通欄 | 追加:
    ...
    if 状態["版"].endswith(("-v4", "-v5", "-v6")):
        旧番号 = int(状態["版"].rsplit("v", 1)[1])
        許容 = {"HDS-MINIDORA-全体運用-v" + str(番号) for 番号 in range(1, 旧番号)}
        if (type(状態["移行履歴"]) is not list or any(type(行) is not dict or 行.get("旧版") not in 許容
                for 行 in 状態["移行履歴"])):
            raise ValueError(状態["版"].split("-")[-1] + "当時に存在しない移行履歴")
        # v4/v5には移行記録v1のみ、v6にはv1/v2が存在した。未来版を後付けしない。
        記録版 = {"HDS旧保存明示移行-v1"}
        if 旧番号 == 6:
            記録版.add("HDS旧保存明示移行-v2")
        移行履歴を検査(状態["移行履歴"], 深さ=深さ + 1, 許容版=記録版)
    # 旧形式に未来の横断要求を埋めて現行能力へ流入させない。
    for 鍵 in ("前回目的", "保留目的"):
        目的 = 状態.get(鍵)
        if type(目的) is dict and 目的.get("方式") == "関係資料":
            要求 = 目的.get("要求")
            期待版 = "HDS関係説明要求-v2" if 状態["版"].endswith("-v6") else "HDS関係説明要求-v1"
            if (not 状態["版"].endswith(("-v5", "-v6")) or type(要求) is not dict
                    or set(要求) != {"版", "対象", "範囲", "問い", "形式", "不足調査", "最大文字数"}
                    or 要求.get("版") != 期待版):
                raise ValueError("旧保存当時に存在しない関係要求")
    return 状態
...
def 旧保存を移行(原本, *, 外部読取許可=False, 取得器=None):
    ...
    if 旧["版"].endswith("-v1"):
        ...
        除外 = set()
        if not 旧["版"].endswith(("-v5", "-v6")):
            除外 |= set(関係能力名)
        if not 旧["版"].endswith(("-v4", "-v5", "-v6")):
    ...
    if 旧["版"].endswith(("-v5", "-v6")):
        # v5/v6の旧目的は通常関係要求だけ。新しい横断目的を旧成果から生成しない。
