...
class HDSJudge:
    def decide(self, frame: SemanticFrame, candidates: Sequence[候補]) -> "JudgeDecision":
...
@dataclass(frozen=True)
class SystemResult:
    status: str; text: str; answer: str | None; decision: JudgeDecision; trace: tuple[dict[str, Any], ...]; candidates: tuple[候補, ...]
...
class K3相当能力核:
    ...
    def _fact_candidates(self, frame: SemanticFrame, policy: EffortPolicy) -> tuple[list[候補], str]:
        ...
        for fact in facts:
            ...
            candidates.append(候補(self.R.label(answer_key),fact.predicate,fact.confidence,expert,tuple(x.fact_id for x in proof),fact.provenance,tuple(contradictions)))
    ...
    def _emit(self, frame: SemanticFrame, decision: JudgeDecision, candidates: Sequence[候補], trace: list[dict[str, Any]]) -> SystemResult:
