...
def _combined_confidence(state: 値状態, source_confidence: float) -> float:
    return _confidence(state) * _情報源信頼度(source_confidence)


def _状態印(state: 値状態) -> str:
...
def _関係条件(relation: HDS関係, key: str) -> str:
    prefix = key + "="
...
def _関係極性(relation: HDS関係) -> bool:
    value = _関係条件(relation, "極性")
    return value != "否定"


def _関係修飾(relation: HDS関係) -> tuple[tuple[str, str], ...]:
...
class HDSIR知識Adapter:
    ...
    def 投入(
        ...
        *,
        ...
        source_confidence = _情報源信頼度(信頼係数)
        source_marker = _情報源印(source_confidence)
        coords = ir.座標辞書()
        facts: list[Fact] = []
        ...
        for coord in ir.座標:
            ...
            fact = Fact(
                ...
                provenance=source + ("HDS-IR", coord.座標ID, _状態印(coord.値状態), source_marker, *residual_markers, _text(coord.由来), _text(coord.暫定性)),
            )
            facts.append(fact)
        ...
        for relation in ir.関係:
            ...
            if residual_markers:
                blocked_count += 1
            polarity = _関係極性(relation)
            qualifiers = _関係修飾(relation)
            provenance_value = source + (
                "HDS-IR", relation.関係ID, _状態印(relation.値状態), source_marker,
