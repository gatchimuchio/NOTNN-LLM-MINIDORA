...
def 回答を構成(values: tuple[能力結果,...], *, 詳細=False, 最大文字数=100000):
...
def 回答記録整合(value):
    try:
        d=value.データ
        rebuilt=回答を構成(tuple(能力結果を復元(v) for v in d['元結果']),詳細=d['詳細'],最大文字数=d['最大文字数'])
        return value.成立 and rebuilt.データ==d and rebuilt.本文==value.本文
    except (KeyError,ValueError,TypeError,AttributeError,RecursionError):
        return False


def 回答を再表現(value, *, 詳細=False):
