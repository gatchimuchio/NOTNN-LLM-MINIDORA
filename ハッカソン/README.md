...
> **ハッカソンでは「今日のニュースは？ → 要約して」という短い対話を通じて、MINIDORAの外部参照・会話状態・能力能力モジュール・応答追跡を一つの製品動作として見せます。**

本ディレクトリはハッカソン提出・デモ用の運用境界を記録します。実装本体は [`../src/minidora/ハッカソン/`](../src/minidora/ハッカソン/) に置き、既存MINIDORA 模型核を改変せず能力能力モジュールとして接続します。
...
- 既存MINIDORA 模型核が実行記録APIを持つ場合、`結果 / 参照 / 履歴 / 採否 / 言語計画 / HDS_IR` を監査記録へ保持
- 任意の追記専用JSONL監査保存
...
- 能力モジュールへの入力と出力
- 会話状態の参照・更新
- MINIDORA 模型核へ委譲した場合の公開実行記録
- 最終応答
- 応答状態
- 段階hash / root hash

一般質問を既存MINIDORA 模型核へ委譲する場合は、模型核が公開する実行結果・参照・履歴・採否・HDS_IRを同じ監査鎖へ取り込みます。
...
| 実装 | 責任 |
|---|---|
| `チャット.py` | 能力経路選択、既存MINIDORA 模型核委譲、監査接続 |
...
v0.2はチャット模型核と監査境界を先に成立させた段階です。次は次の配信・表示層を接続します。
