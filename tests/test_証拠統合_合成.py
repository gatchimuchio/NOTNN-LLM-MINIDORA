...
def plan_and_資料(texts, *, settings=None):
    ...
    return 合成計画(stages, ("抽出",)), 資料


class 証拠合成接続試験(unittest.TestCase):
    def setUp(self):
        self.runner = 能力合成器((証拠統合モジュール().登録(), 記載値採用モジュール().登録(), *局所能力群()))

    def run_texts(self, *texts, settings=None):
        p,d = plan_and_資料(texts, settings=settings)
    ...
    def test_参照ではなく前段の生成文を根拠にしない(self):
        p,d = plan_and_資料(["装置Aの電圧は120 Vです。"])
    ...
    def test_資料の順序が変わっても採用できる(self):
        p,d = plan_and_資料(["装置Aの電圧は120 Vです。", "装置Aの電圧は0.12 kVです。"])
    ...
    def test_採用関門を省略しても報告は診断として読める(self):
        p,d = plan_and_資料(["装置Aの電圧は120 Vです。", "装置Aの電圧は240 Vです。"])
    ...
    def test_採用モジュールに通常の文章を渡しても不成立(self):
        m = 記載値採用モジュール()
...
class 取得証拠接続試験(unittest.TestCase):
    def 処理系列(self, second):
        取得=知識取得モジュール(知識取得器(_検索(),_HTML供給(second)),外部読取許可=True)
        runner=能力合成器((取得.登録(),証拠統合モジュール().登録(),記載値採用モジュール().登録(),*局所能力群()))
        p,d=plan_and_資料([])
    ...
    def test_独立CLIの一致競合と入力摂動(self):
        ...
        for args in ([],["--競合"],["--値","731"]):
            with self.subTest(args=args):
                結果=subprocess.run([sys.executable,str(root/"tools/証拠統合デモ.py"),*args],
                    capture_output=True,encoding="utf-8",timeout=15)
                self.assertEqual(結果.returncode,0,結果.stderr)
                output=json.loads(結果.stdout)
