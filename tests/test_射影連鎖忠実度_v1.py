...
def _cond(関係: HDS関係, key: str) -> str:
    prefix = key + "="
    for raw in 関係.条件:
        value = str(raw)
...
class 三層射影忠実度試験(unittest.TestCase):
    ...
    def test_英語宣言関係射影は否定と条件を落とさない(self) -> None:
        ...
        関係 = next(r for r in projected.関係 if _cond(r, "検索述語") == "alter")
        self.assertEqual(関係.種別, "開放述語")
        self.assertEqual(_cond(関係, "極性"), "否定")
        self.assertIn("condition C", _cond(関係, '条件範囲'))
    ...
    def test_候補代入は問いの極性と範囲を保持する(self) -> None:
        ...
        hypotheses = HDS候補代入仮説群(kq, 候補_irs)
        for 候補 in hypotheses.values():
            関係 = next(r for r in 候補.関係 if r.由来 == "HDS候補代入仮説")
            self.assertEqual(_cond(関係, "極性"), "否定")
            self.assertIn("condition C", _cond(関係, '条件範囲'))
    ...
    def test_正式模型核はHDS非依存の参照寄与で候補差を形成する(self) -> None:
        question = _ir(
            ...
            (HDS座標('未知', "目的.未知始点", "molecule", 値状態.未観測), HDS座標("target", "対象.終点", "Enzyme X")),
            (HDS関係("q", ('未知',), ("target",), "阻害", 条件=("検索述語=inhibit", "不足位置=始点"), 値状態=値状態.確定),),
        ...
        結果 = HDSMINIDORA模型評価(question, hypotheses, (証拠,), 模型核=標準模型核())
        self.assertEqual(結果.状態, "APPROVE")
        self.assertEqual(結果.回答ラベル, "A")
        self.assertTrue(結果.模型結果.検査点)
        ...
        reverse_question = _ir(
            ...
            question.座標,
            (HDS関係("q", ('未知',), ("target",), "阻害", 条件=("検索述語=inhibit", "不足位置=始点", "選択意図=反転"), 値状態=値状態.確定),),
            kind="knowledge_query",
        )
        contradicted = _ir(
        ...
        reverse = HDSMINIDORA模型評価(reverse_question, reverse_hypotheses, (証拠, contradicted), 模型核=標準模型核())
    ...
    def test_選択問題最終閉包は通常意味コンパイルへ漏れない(self) -> None:
        構文化器 = 公開HDSコンパイラ()
        ordinary = 構文化器.意味コンパイル("Find the energy spectrum.")
    ...
    def test_未解析だが意味内容のある質問は問い適合として保持する(self) -> None:
        ...
        self.assertFalse(any(r.種別 == '意味_loss' for r in unresolved.残差))

    def test_正式模型候補代入は質問型を使い旧K3判定を変えない(self) -> None:
        quantity_question = _ir(
            "How many products?",
            (HDS座標('未知', "目的.未知終点", "数量", 値状態.未観測), HDS座標("known", "対象.始点", "products")),
            (HDS関係("q", ("known",), ('未知',), "数量同定", 条件=("不足位置=終点", "検索述語=count"), 値状態=値状態.未観測),),
            kind="knowledge_query",
        )
        ...
        generic_question = _ir(
            ...
            (HDS座標('未知', "目的.未知始点", "選択肢", 値状態.未観測), HDS座標("known", "対象.問い本文", "energy spectrum")),
            (HDS関係("q", ('未知',), ("known",), "問い適合", 条件=("不足位置=始点", "選択問題閉包=v0.1"), 値状態=値状態.未観測),),
            kind="knowledge_query",
        )
        formula = _ir(
            "E = 2 n + 1",
        ...
        world_question = _ir(
            ...
            (HDS座標('未知', "目的.未知始点", "molecule", 値状態.未観測), HDS座標("x", "対象.終点", "X")),
            (HDS関係("q", ('未知',), ("x",), "阻害", 条件=("不足位置=始点",), 値状態=値状態.未観測),),
            kind="knowledge_query",
        )
        proposition = _ir(
            "A activates B",
    ...
    def test_正式模型候補代入は意味_loss候補を強制採用しない(self) -> None:
        question = _ir(
            ...
            (HDS座標('未知', "目的.未知始点", "選択肢", 値状態.未観測), HDS座標("known", "対象.問い本文", "explanation")),
            (HDS関係("q", ('未知',), ("known",), "説明適合", 条件=("不足位置=始点",), 値状態=値状態.未観測),),
            kind="knowledge_query",
        )
        broken = HDSIR(
            原文="opaque", 正規化文="opaque", 認知世界ID='忠実度-test',
            座標=(HDS座標("x", "対象.実体", "opaque"),), 関係=(),
            残差=(HDS残差("loss", '意味_loss', "opaque", "unresolved"),),
            意味作用履歴=(), 実行核=HDS実行核(), 種別="一般", 入力言語="en",
        )
        self.assertFalse(HDS模型候補代入可能(question, broken))

    def test_正式模型適合器は同一問題を質問側言語体系へ統一する(self) -> None:
        question = _ir(
            "Which molecule inhibits Enzyme X?",
            (HDS座標('未知', "目的.未知始点", "molecule", 値状態.未観測), HDS座標("target", "対象.終点", "Enzyme X")),
            (HDS関係("q", ('未知',), ("target",), "阻害", 条件=("検索述語=inhibit", "不足位置=始点"), 値状態=値状態.確定),),
        ...
        candidates = {
            "A": _ir("候補A", (HDS座標("a", "対象.実体", "Molecule A"),), 言語="ja"),
            "B": _ir("Molecule B", (HDS座標("b", "対象.実体", "Molecule B"),), 言語="en"),
        }
        hypotheses = HDS候補代入仮説群(question, candidates)
        証拠 = _ir(
            "参照",
        ...
        結果 = HDSMINIDORA模型評価(question, hypotheses, (証拠,), 模型核=標準模型核())
        self.assertIn(結果.状態, {"APPROVE", "SUSPEND"})
        self.assertEqual(結果.模型結果.文脈.現在.言語体系, "自然言語:en")
        self.assertTrue(all(item.言語体系 == "自然言語:en" for item in 結果.模型結果.文脈.参照状態))
