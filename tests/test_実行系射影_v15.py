...
from minidora.トリニティ文脈 import Trinity文脈系
...
class Runtime射影V15試験(unittest.TestCase):
    ...
    def test_未知端点でも既知関係種別をKへ渡す(self) -> None:
        original = self.構文化器.コンパイル("Which molecule inhibits Enzyme X?")
        projected = HDSK質問射影(original)
        関係 = next(r for r in projected.関係 if _条件値(r, "不足位置") == "始点")
        self.assertEqual(str(関係.種別), "阻害")
        self.assertEqual(関係.値状態, 値状態.確定)
        self.assertEqual(str(関係.暫定性), '関係_TYPE_KNOWN_ENDPOINT_OPEN')
        coords = projected.座標辞書()
        self.assertTrue(any(coords[cid].内容 == "Enzyme X" for cid in 関係.終点 if cid in coords))
        self.assertTrue(any(coords[cid].値状態 == 値状態.未観測 for cid in 関係.始点 if cid in coords))

    def test_関係質問では検索制御メタをKへ混ぜない(self) -> None:
        original = self.構文化器.問題IR(
            "Which molecule is least likely to inhibit Enzyme X?",
            ("A", "B", "C", "D"),
        )
        projected = HDSK質問射影(original)
        kinds = {str(c.種別) for c in projected.座標 if not c.座標ID.startswith('選択肢:')}
    ...
    def test_説明選択質問はtopic_onlyへ落とさず問い適合関係を使う(self) -> None:
        ...
        projected = HDSK質問射影(original)
        関係 = next(r for r in projected.関係 if _条件値(r, "不足位置") == "始点")
        self.assertEqual(str(関係.種別), "説明適合")
        self.assertEqual(_条件値(関係, "検索述語"), "describe")
        self.assertFalse(any(残差.種別 == '意味_loss' for 残差 in projected.残差))
    ...
    def test_R関係質問は検索述語既知端点候補を保持する(self) -> None:
        ...
        for 候補 in ("compound a", "compound b", "compound c", "compound d"):
            self.assertTrue(any(候補 in q and "inhibit" in q and "enzyme x" in q for q in queries), queries)

    def test_R射影は選択極性と完全質問文を検索代替経路へ残さない(self) -> None:
        original = self.構文化器.問題IR(
            "Which molecule is least likely to inhibit Enzyme X?",
            ("Compound A", "Compound B", "Compound C", "Compound D"),
        )
    ...
    def test_R一般質問は検索表層だけを主入力にする(self) -> None:
        ...
        projected = HDSR質問射影(original)
        nonchoice = [c for c in projected.座標 if not c.座標ID.startswith('選択肢:')]
    ...
    def test_候補と資料では検索制御目的監査をKへ入れない(self) -> None:
        ir = HDSIR(
            原文="synthetic",
            正規化文="synthetic",
            認知世界ID="test",
        ...
        for projected in (HDSK候補射影(ir), HDSK資料射影(ir)):
    ...
    def test_実体候補だけ未知端点へ代入可能とする(self) -> None:
        ...
        self.assertFalse(HDSK候補代入可能(_合成関係中間表現()))

    def test_Kは明示肯定関係だけを有向Fact候補へ残す(self) -> None:
        projected = HDSK資料射影(_合成関係中間表現())
        self.assertEqual(len(projected.関係), 1)
        self.assertEqual(projected.関係[0].種別, "阻害")

    def test_Kは否定範囲を肯定関係へ潰さず極性付きで保持する(self) -> None:
        projected = HDSK資料射影(_合成関係中間表現(conditions=("極性=否定",)))
        self.assertEqual(len(projected.関係), 1)
        self.assertEqual(projected.関係[0].種別, "阻害")
        self.assertEqual(_条件値(projected.関係[0], "極性"), "否定")

    def test_Kは様相範囲を無条件化せず修飾付き関係として保持する(self) -> None:
        projected = HDSK資料射影(_合成関係中間表現(conditions=("様相=可能",)))
        self.assertEqual(len(projected.関係), 1)
        self.assertEqual(_条件値(projected.関係[0], "様相"), "可能")
        self.assertTrue(any(str(c.内容) == "Compound A" for c in projected.座標))

    def test_Kは条件範囲を無条件化せず修飾付き関係として保持する(self) -> None:
        projected = HDSK資料射影(_合成関係中間表現(conditions=('条件範囲=if condition X',)))
        self.assertEqual(len(projected.関係), 1)
        self.assertEqual(_条件値(projected.関係[0], '条件範囲'), "if condition X")
