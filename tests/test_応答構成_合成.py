...
from minidora.応答構成接続 import 応答構成モジュール
...
def 計画と資料(settings=None):
    report=報告("装置Aの電圧は120 Vです。","装置Aの電圧は0.12 kVです。")
    資料={"報告":report,"指示":能力結果(True,"回答を構成"),
...
class 応答合成接続試験(unittest.TestCase):
    def setUp(self):
        self.runner=能力合成器((応答構成モジュール().登録(),))
    ...
    def test_停止要求は回答モジュールの実行前に観測(self):
        p,d=計画と資料()
...
class 取得比較回答接続試験(unittest.TestCase):
    def 処理系列(self,n=120,conflict=False,残差=False):
        取得=知識取得モジュール(知識取得器(_検索(),_本文(n,conflict,残差)),外部読取許可=True)
        runner=能力合成器((取得.登録(),証拠統合モジュール().登録(),応答構成モジュール().登録()))
        資料={"取得指示":能力結果(True,"資料取得"),
    ...
    def test_独立CLIの一致競合留保と入力摂動(self):
        ...
        for args in ([],["--競合"],["--留保"],["--値","731"],["--形式","箇条書き","--詳細度","詳細"]):
            with self.subTest(args=args):
                ...
                self.assertEqual(資料["状態"],"合格")
                self.assertTrue(資料["回答監査"] and 資料["合成監査"])
                self.assertEqual(資料["項目状態"][0]["記載値採用可"],not ("--競合" in args or "--留保" in args))
