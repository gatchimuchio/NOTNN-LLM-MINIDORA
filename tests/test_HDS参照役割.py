from __future__ import annotations

import unittest

from minidora.HDS中間表現 import HDSIR, HDS実行核, HDS座標, HDS関係, 値状態
from minidora.HDS参照 import HDS参照問合せ候補


def _ir() -> HDSIR:
    return HDSIR(
        原文="Which process is active for ProteinX under hypoxia?",
        正規化文="Which process is active for ProteinX under hypoxia?",
        認知世界ID="参照-役割-test",
        座標=(
            HDS座標("unknown", "目的.未知始点", "process", 値状態.未観測),
            HDS座標("protein", "対象.実体", "ProteinX"),
            HDS座標("search", "検索.英語正規化", "ProteinX activates under hypoxia acute phase"),
            HDS座標("uncertain", "条件.未解", "mouse only", 値状態.未確定),
            HDS座標("選択肢:A", "目的.候補", "catalysis"),
            HDS座標("選択肢:B", "目的.候補", "transport"),
            HDS座標("選択肢:C", "目的.候補", "folding"),
            HDS座標("選択肢:D", "目的.候補", "signaling"),
        ),
        関係=(
            HDS関係(
                "question",
                ("unknown",),
                ("protein",),
                "関連",
                条件=("検索述語=activates", "不足位置=始点", "条件範囲=under hypoxia", "時間範囲=acute phase"),
                値状態=値状態.未観測,
            ),
        ),
        残差=(),
        意味作用履歴=(),
        実行核=HDS実行核("参照回答"),
        参照必須=True,
        種別="knowledge_query",
        閉包状態="作用閉包",
        入力言語="en",
    )


def _formula_ir() -> HDSIR:
    common = "The mass dimension of kappa is"
    return HDSIR(
        原文="What is the mass dimension of kappa and is the theory renormalizable?",
        正規化文="What is the mass dimension of kappa and is the theory renormalizable?",
        認知世界ID="参照-formula-test",
        座標=(
            HDS座標("unknown", "目的.未知始点", "property", 値状態.未観測),
            HDS座標("subject", "対象.実体", "kappa interaction operator"),
            HDS座標("選択肢:A", "目的.候補", f"{common} -1 and the theory is renormalizable"),
            HDS座標("選択肢:B", "目的.候補", f"{common} -1 and the theory is not renormalizable"),
            HDS座標("選択肢:C", "目的.候補", f"{common} 1 and the theory is renormalizable"),
            HDS座標("選択肢:D", "目的.候補", f"{common} 1 and the theory is not renormalizable"),
        ),
        関係=(
            HDS関係(
                "question",
                ("unknown",),
                ("subject",),
                "属性",
                条件=("検索述語=property", "不足位置=始点", "条件範囲=mass dimension renormalizable"),
                値状態=値状態.未観測,
            ),
        ),
        残差=(),
        意味作用履歴=(),
        実行核=HDS実行核("参照回答"),
        参照必須=True,
        種別="knowledge_query",
        閉包状態="作用閉包",
        入力言語="en",
    )


class HDS参照役割Query試験(unittest.TestCase):
    def test_HDS役割順でKernel形成済みqueryを降下する(self) -> None:
        queries = HDS参照問合せ候補(_ir())
        structural = next(
            q for q in queries
            if "ProteinX" in q and "activates" in q and "hypoxia" in q and "acute phase" in q
        )
        self.assertLess(structural.index("ProteinX"), structural.index("activates"))
        self.assertNotIn("mouse only", structural)

    def test_4択query枠を全候補へ対称に予約する(self) -> None:
        queries = HDS参照問合せ候補(_ir(), 最大候補数=4)
        self.assertEqual(len(queries), 4)
        for 選択肢 in ("catalysis", "transport", "folding", "signaling"):
            self.assertEqual(sum(選択肢 in q for q in queries), 1)
        self.assertTrue(all("ProteinX" in q for q in queries))

    def test_既定6枠では関係anchorと4候補を共存させる(self) -> None:
        queries = HDS参照問合せ候補(_ir())
        self.assertEqual(len(queries), 6)
        self.assertEqual(sum("catalysis" in q for q in queries), 1)
        self.assertEqual(sum("transport" in q for q in queries), 1)
        self.assertEqual(sum("folding" in q for q in queries), 1)
        self.assertEqual(sum("signaling" in q for q in queries), 1)

    def test_長文選択肢は候補差分を検索へ残す(self) -> None:
        queries = HDS参照問合せ候補(_formula_ir(), 最大候補数=4)
        self.assertEqual(len(queries), 4)
        self.assertTrue(any("-1" in q for q in queries))
        self.assertTrue(any("not" in q for q in queries))
        self.assertTrue(all(len(q) <= 360 for q in queries))

    def test_数値だけが違う選択肢でも差分を失わない(self) -> None:
        ir = HDSIR(
            原文="How many loops?",
            正規化文="How many loops?",
            認知世界ID="参照-number-test",
            座標=(
                HDS座標("unknown", "目的.未知始点", "count", 値状態.未観測),
                HDS座標("subject", "対象.実体", "loop diagram"),
                HDS座標("選択肢:A", "目的.候補", "1"),
                HDS座標("選択肢:B", "目的.候補", "2"),
                HDS座標("選択肢:C", "目的.候補", "3"),
                HDS座標("選択肢:D", "目的.候補", "6"),
            ),
            関係=(
                HDS関係(
                    "question", ("unknown",), ("subject",), "数量",
                    条件=("検索述語=loops", "不足位置=始点"), 値状態=値状態.未観測,
                ),
            ),
            残差=(), 意味作用履歴=(), 実行核=HDS実行核("参照回答"),
            参照必須=True, 種別="knowledge_query", 閉包状態="作用閉包", 入力言語="en",
        )
        queries = HDS参照問合せ候補(ir, 最大候補数=4)
        for number in ("1", "2", "3", "6"):
            self.assertEqual(sum(number in q.split() for q in queries), 1)

    def test_長文検索表層は冒頭文脈と末尾焦点を同時に保持する(self) -> None:
        filler = " ".join(f"context{i}" for i in range(120))
        value = f"AlphaProtein {filler} final_focus_marker"
        ir = HDSIR(
            原文=value,
            正規化文=value,
            認知世界ID="参照-long-文脈-test",
            座標=(
                HDS座標("search", "検索.英語正規化", value),
                HDS座標("選択肢:A", "目的.候補", "red"),
                HDS座標("選択肢:B", "目的.候補", "blue"),
            ),
            関係=(), 残差=(), 意味作用履歴=(), 実行核=HDS実行核("参照回答"),
            参照必須=True, 種別="knowledge_query", 閉包状態="作用閉包", 入力言語="en",
        )
        queries = HDS参照問合せ候補(ir)
        self.assertTrue(any("AlphaProtein" in q and "final_focus_marker" in q for q in queries))

    def test_形成済み焦点検索表層を背景と別queryでも保持する(self) -> None:
        background = " ".join(f"background{i}" for i in range(90))
        focus = "Which mechanism controls the final state?"
        value = f"AlphaProtein is observed in the experiment. {background} {focus}"
        ir = HDSIR(
            原文=value,
            正規化文=value,
            認知世界ID="参照-focus-test",
            座標=(
                HDS座標("search:background", "検索.英語正規化", value),
                HDS座標("search:focus", "検索.焦点", focus),
                HDS座標("選択肢:A", "目的.候補", "catalysis"),
                HDS座標("選択肢:B", "目的.候補", "transport"),
                HDS座標("選択肢:C", "目的.候補", "folding"),
                HDS座標("選択肢:D", "目的.候補", "signaling"),
            ),
            関係=(), 残差=(), 意味作用履歴=(), 実行核=HDS実行核("参照回答"),
            参照必須=True, 種別="knowledge_query", 閉包状態="作用閉包", 入力言語="en",
        )
        queries = HDS参照問合せ候補(ir)
        self.assertIn(focus, queries)


if __name__ == "__main__":
    unittest.main()
