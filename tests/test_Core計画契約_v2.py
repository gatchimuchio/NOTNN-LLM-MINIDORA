...
class Core要求被覆試験(unittest.TestCase):
    ...
    def test_出力へ接続しない目的をRequestIR境界で拒否(self):
        ...
        with self.assertRaisesRegex(ValueError,'出力に接続していない目的'):
            replace(r,目的=(*r.目的,extra),引数Data={**r.引数Data,'p3':{}}).固定複製()
...
class Core作用契約試験(unittest.TestCase):
    def setUp(self):
        self.registry=({'名前':'合成','版':'v1','外部読取':False},{'名前':'代替','版':'v1','外部読取':False})
    ...
    def test_ActionContract_v2は複数素材と実体契約を被覆する(self):
        self.assertEqual(役割計画版,'MINIDORA-役割計画-v0.3')
        result=役割計画器((self.rule,),self.registry).計画する(self.goal,self.materials)
        self.assertEqual({x.素材 for x in result.要求被覆 if x.解決=='素材'},{'A','B'})
        root=next(x for x in result.要求被覆 if x.目的鍵==self.goal.鍵())
        self.assertEqual(tuple(n for n,_ in root.入力役割),('左','右'))
        self.assertTrue(root.契約印);self.assertTrue(result.作用契約印)

    def test_設定関数を探索後に再評価しない(self):
        calls={'n':0}
        def settings(p): calls['n']+=1;return {'呼出':calls['n']}
        result=役割計画器((replace(self.rule,設定=settings),),self.registry).計画する(self.goal,self.materials)
        self.assertEqual(calls['n'],1)
        config=next(v for k,v in result.Data.items() if k.startswith('設定:'))
        self.assertEqual(config.データ,{'呼出':1})

    def test_同一入力は同一の被覆と作用契約印(self):
        p=役割計画器((self.rule,),self.registry)
    ...
    def test_保持事項の重複を契約登録時に拒否(self):
        with self.assertRaisesRegex(ValueError,'保持事項'):
            役割計画器((replace(self.rule,保持事項=('対象','対象')),),self.registry)


class _統合Stub:
    def __init__(self,registry): self.registry=registry;self.calls=0;self.state=('s',0)
    def 起点(self): return self.state
    def 能力一覧(self): return self.registry
    def _停止(self,stop):
        if stop and stop(): raise InterruptedError()
    def 準備(self,plan,data,依頼文=''): return SimpleNamespace(起点=self.state,ハッシュ='plan-'+str(self.calls+1))
...
class Core失敗駆動再計画試験(unittest.TestCase):
    def setUp(self):
        self.registry=({'名前':'A','版':'v1','外部読取':False},{'名前':'B','版':'v1','外部読取':False})
    ...
    def test_再計画ごとに被覆と実体契約を監査する(self):
        planner=役割計画器((self.first,self.second),self.registry);root=_統合Stub(self.registry)
        result=会話実行監督(planner,root).実行(self.goal,{},原文='成果')
        self.assertTrue(result.応答.成立);self.assertEqual(len(result.試行),2)
        self.assertTrue(all(x['要求被覆印'] and x['作用契約印'] for x in result.試行))
        self.assertNotEqual(result.試行[0]['作用契約印'],result.試行[1]['作用契約印'])
        self.assertEqual(result.失敗[0].再開放,(self.goal.鍵(),'第一'))

    def test_被覆欠落計画を実行前に拒否する(self):
        planner=役割計画器((self.second,),self.registry);original=planner.計画する
        planner.計画する=lambda *a,**k: replace(original(*a,**k),要求被覆=())
        root=_統合Stub(self.registry)
    ...
    def test_再計画で失敗していない同一作用の契約を横滑りさせない(self):
        ...
        planner=役割計画器((root_rule,leaf_a,leaf_b),self.registry)
        integration=_統合Stub(self.registry)
        # 最初の失敗工程を子工程へ合わせる。親「終端」は失敗していないのに、
        # 再計画で同じ作用の設定だけが変わるなら実行前に停止する。
        original_execute=integration.実行
        ...
        integration.実行=execute
        with self.assertRaisesRegex(ValueError,'既存作用契約'):
            会話実行監督(planner,integration).実行(意味目的('成果',{}),{},原文='成果')
