...
class HDSGraph単純路試験(unittest.TestCase):
    def test_直接到達を往復循環で水増ししない(self) -> None:
        ...
        adapter.投入(_関係中間表現("engine"), provenance=("fixture", "doc:engine"))
        adapter.投入(_関係中間表現("stone"), provenance=("fixture", "doc:stone"))
    ...
    def test_正当な多段単純路は維持する(self) -> None:
        core = K3相当能力核()
        adapter = HDSIR知識Adapter(core)
        adapter.投入(_関係中間表現("Beta"), provenance=("fixture", "doc:1"))
        second = HDSIR(
    ...
    def test_graph索引はK更新まで再利用し投入後に無効化する(self) -> None:
        core = K3相当能力核()
        adapter = HDSIR知識Adapter(core)
        adapter.投入(_関係中間表現("engine"), provenance=("fixture", "doc:1"))

        first = HDS意味Graph索引構築(core)
        second = HDS意味Graph索引構築(core)
        self.assertIs(first, second)

        adapter.投入(_関係中間表現("stone"), provenance=("fixture", "doc:2"))
