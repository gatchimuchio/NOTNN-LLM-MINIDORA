...
class 多言語TrinityFixture構文化器:
    def コンパイル(self, 入力, *, 前回結果=None, HDS履歴=(), 文脈: HDS文脈 | None = None):
        言語 = "ja"
        if any(字句 in 入力 for 字句 in ("What", "Add", "sum", "plus", "it")):
            言語 = "en"
        elif any(字句 in 入力 for 字句 in ("总和", "给", "它", "多少")):
            言語 = "zh"

        followup = any(字句 in 入力 for 字句 in ("それ", "it", "它"))
        if followup:
            focus = 文脈.現在焦点 if 文脈 is not None else 前回結果
            if focus is None:
                return _未閉包IR(入力, 言語=言語)
            refs = 文脈.記憶引用 if 文脈 is not None else ('legacy:last_結果',)
            return _加算IR(入力, focus, 4, 言語=言語, 文脈引用=refs)

        return _加算IR(入力, 2, 3, 言語=言語)
...
class 多言語Trinity試験(unittest.TestCase):
    def test_日本語英語中国語が同じHDS実行核へ落ちる(self):
        body = ミニドラ(HDSコンパイラ_=多言語TrinityFixture構文化器())
        cases = (
            ("2と3の和は？", "ja"),
        ...
        signatures = []
        for text, 言語 in cases:
            ir = body.コンパイル(text)
            self.assertEqual(ir.入力言語, 言語)
            signatures.append((ir.実行核.作用, tuple(ir.初期状態.values())))
        self.assertEqual(set(signatures), {("加算", (2, 3))})

    def test_各入力言語へ自然言語出力を戻す(self):
        self.assertEqual(ミニドラ(HDSコンパイラ_=多言語TrinityFixture構文化器()).応答("2と3の和は？"), "5です。")
        self.assertEqual(ミニドラ(HDSコンパイラ_=多言語TrinityFixture構文化器()).応答("What is the sum of 2 and 3?"), "5.")
        self.assertEqual(ミニドラ(HDSコンパイラ_=多言語TrinityFixture構文化器()).応答("2和3的总和是多少？"), "5。")
    ...
    def test_Trinity_MからJが現在焦点を引用して言語跨ぎで継続する(self):
        ...
        self.assertIn('作業:current_focus', body.HDS文脈.記憶引用)
        self.assertEqual(len(body.HDS履歴), 4)
        self.assertIn('作業:current_focus', body.HDS履歴[-1].文脈引用)

    def test_文脈参照先なしは捏造せず保留(self):
        body = ミニドラ(HDSコンパイラ_=多言語TrinityFixture構文化器())
        結果 = body.実行(要求("Add 4 to it"))
        self.assertEqual(結果.採否.状態, 実行状態.保留)
        self.assertIsNone(結果.値)
        self.assertEqual(結果.HDS_IR.入力言語, "en")

    def test_未閉包残差をMが次turn文脈として保持する(self):
        body = ミニドラ(HDSコンパイラ_=多言語TrinityFixture構文化器())
        body.実行(要求("Add 4 to it"))
        self.assertTrue(body.HDS文脈.未解残差)
        self.assertIn('作業:unresolved', body.HDS文脈.記憶引用)

    def test_旧式構文化器も文脈引数なしで継続利用できる(self):
        body = ミニドラ(HDSコンパイラ_=旧式Fixture構文化器())
