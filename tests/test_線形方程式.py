...
class 線形方程式試験(unittest.TestCase):
    def solve(self, *equations, names=('x','y')):
        結果=線形を解く(names,equations)
        self.assertTrue(結果.成立,(結果.保留理由,結果.データ))
        self.assertTrue(数学記録整合(結果))
        return 結果

    def test_連立一次方程式の一意解(self):
        結果=self.solve(eq('x+y','5'),eq('x-y','1'))
        self.assertEqual(結果.データ['判定'],'一意解')
        self.assertEqual(結果.データ['解'],{'x':'3','y':'2'})

    def test_入力係数変更が解へ到達(self):
        for n in (9,731,10007):
            結果=self.solve(eq('x+y',str(n)),eq('x-y','1'))
            self.assertEqual(F(結果.データ['解']['x']),(F(n)+1)/2)

    def test_有理数と十進数の正確な解(self):
        結果=self.solve(eq('0.1*x+0.2*y','0.3'),eq('x-y','0'))
        self.assertEqual(結果.データ['解'],{'x':'1','y':'1'})

    def test_先頭零の行交換(self):
        結果=self.solve(eq('y','2'),eq('x','3'))
        self.assertEqual(結果.データ['解'],{'x':'3','y':'2'})
        self.assertTrue(any(x['操作']=='交換' for x in 結果.データ['行操作']))

    def test_条件不足では自由変数を勝手に固定しない(self):
        結果=self.solve(eq('x+y','5'))
        self.assertEqual(結果.データ['判定'],'自由解')
        self.assertIsNone(結果.データ['解'])
        self.assertEqual(結果.データ['自由変数'],['y'])
        self.assertEqual(結果.データ['特解'],{'x':'5','y':'0'})
        self.assertEqual(結果.データ['基底'][0]['係数'],{'x':'-1','y':'1'})

    def test_自由変数を含む族を元式に独立代入(self):
        結果=self.solve(eq('x+2*y-z','3'),names=('x','y','z'))
        self.assertEqual(結果.データ['判定'],'自由解')
        for s,t in product((F(-2),F(1,3),F(7)),repeat=2):
            values={name:F(v) for name,v in 結果.データ['特解'].items()}
            for parameter,basis in zip((s,t),結果.データ['基底']):
    ...
    def test_冗長行を矛盾にしない(self):
        結果=self.solve(eq('x+y','5'),eq('2*x+2*y','10'))
        self.assertEqual(結果.データ['判定'],'自由解')
        self.assertEqual(結果.データ['階数'],1)

    def test_矛盾を解なしとして保存(self):
        結果=self.solve(eq('x+y','5'),eq('2*x+2*y','11'))
        self.assertEqual(結果.データ['判定'],'解なし')
        proof=結果.データ['矛盾証明']
        self.assertEqual(proof['導出左辺'],['0','0'])
        self.assertNotEqual(F(proof['導出右辺']),0)

    def test_矛盾証明を元の行から独立に再現(self):
        結果=self.solve(eq('x+y','5'),eq('x-y','1'),eq('2*x','7'))
        weights=list(map(F,結果.データ['矛盾証明']['元方程式の係数']))
        rows=[[F(x) for x in r] for r in 結果.データ['元拡大行列']]
    ...
    def test_零方程式と空方程式集合(self):
        for equations in ((),(eq('0','0'),)):
            結果=self.solve(*equations)
            self.assertEqual(結果.データ['判定'],'自由解')
            self.assertEqual(結果.データ['自由変数'],['x','y'])
            self.assertEqual(len(結果.データ['基底']),2)

    def test_定数矛盾の検出(self):
        結果=self.solve(eq('0','1'))
        self.assertEqual(結果.データ['判定'],'解なし')

    def test_多項式の厳密相殺後に線形なら解く(self):
        結果=self.solve(eq('x*x-x*x+x','3'),eq('y','2'))
        self.assertEqual(結果.データ['解'],{'x':'3','y':'2'})

    def test_非線形変数分母不等号を別問題に変換しない(self):
        for equation in (eq('x*y','1'),eq('1/x','2'),eq('x<y','0')):
            結果=線形を解く(('x','y'),(equation,))
            self.assertFalse(結果.成立)
            self.assertNotIn('判定',結果.データ)
...
class 線形独立対照試験(unittest.TestCase):
    ...
    def test_有理三元系は別の行列式展開で対照(self):
        ...
        for t in range(-4,5):
            ...
            結果=線形を解く(('x','y','z'),equations)
            self.assertTrue(結果.成立,結果.データ)
            for j,name in enumerate(('x','y','z')):
                matrix=deepcopy(a)
                for i in range(3):matrix[i][j]=b[i]
                self.assertEqual(F(結果.データ['解'][name]),det(matrix)/determinant)
            count+=1
