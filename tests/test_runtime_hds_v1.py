...
def _question():
    return _ir(
        ...
        (HDS関係("question-use", ("alpha",), ("unknown",), "使用", 条件=("検索述語=use",), 値状態=値状態.未観測),),
        required=True,
    )
...
class Compiler:
    def コンパイル(self, 入力: str, **kwargs):
        if 入力 == "What does Alpha use?": return _question()
        if 入力 in {"engine", "stone"}: return _candidate(入力)
        if 入力 == "Alpha uses engine.": return _data()
        raise ValueError(入力)
...
class RuntimeHDSV1試験(unittest.TestCase):
    def test_実RuntimeでREFERENCE_EVALUATE_COMMITが成立する(self):
        ...
        self.assertEqual(tuple(action for action, _ in run["作用履歴"]), ("REFERENCE", "EVALUATE", "COMMIT"))
