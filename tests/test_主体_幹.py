...
class 主体主幹試験(unittest.TestCase):
    ...
    def test_全処理が同じ主体状態を参照する(self):
        ...
        結果 = self.本体.実行(要求("主体", 手順_))
        self.assertEqual(結果.値["立場"], (("基盤", "K3"),))
        self.assertEqual(結果.主体状態.版, 0)

    def test_理由なし反転は保留し旧状態を維持(self):
        提案 = 主体更新提案(変更={"立場": {"基盤": "Llama3"}})
        結果 = self.本体.実行(要求("基盤変更", self._結果手順(), 主体更新提案=提案))
        self.assertIsNone(結果.値)
        self.assertEqual(結果.採否.状態, 実行状態.保留)
        self.assertEqual(結果.主体状態.立場, (("基盤", "K3"),))
        self.assertEqual(結果.主体状態.版, 0)
    ...
    def test_理由付き訂正は履歴を残して更新(self):
        ...
        結果 = self.本体.実行(要求("仮説更新", self._結果手順(), 主体更新提案=提案))
        self.assertEqual(結果.値, "ok")
        self.assertEqual(結果.採否.状態, 実行状態.合格)
        self.assertEqual(結果.主体状態.版, 1)
        self.assertEqual(結果.主体監査履歴[-1].旧版, 0)
        self.assertEqual(結果.主体監査履歴[-1].新版, 1)
        self.assertTrue(結果.主体監査履歴[-1].理由)
    ...
    def test_Layer0内部からの無言変更も主体主幹を迂回できない(self):
        ...
        結果 = self.本体.実行(要求("迂回", 手順_))
        self.assertIsNone(結果.値)
        self.assertEqual(結果.採否.状態, 実行状態.保留)
        self.assertEqual(結果.主体状態.判断基準, ("根拠なく反転しない",))

    def test_主体IDは処理経路から変更不能(self):
        提案 = 主体更新提案(変更={"主体ID": "OTHER"}, 理由=("変更したい",))
        結果 = self.本体.実行(要求("主体交換", self._結果手順(), 主体更新提案=提案))
    ...
    def test_主体状態はturnを跨いで持続(self):
        ...
        手順_ = 手順("主体参照", (命令("主体取得", 作用.取得, 対象="主体状態", 更新先="結果"),))
        結果 = self.本体.実行(要求("次turn", 手順_))
        self.assertEqual(結果.値["選好"], (("応答言語", "日本語"),))
        self.assertEqual(結果.主体状態.版, 1)
