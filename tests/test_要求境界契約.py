...
class 要求境界契約試験(unittest.TestCase):
    def test_版と決定性(self):
        ...
        coverage=被覆台帳印(rows)
        a=要求境界契約印(原文='現在の依頼',計画印='plan',要求被覆印=coverage,目的印='goal',素材印='data')
        b=要求境界契約印(原文='現在の依頼',計画印='plan',要求被覆印=coverage,目的印='goal',素材印='data')
        self.assertEqual(a,b); self.assertEqual(len(a),64)

    def test_現在依頼_計画_被覆_目的_素材の差を同一視しない(self):
        rows=(被覆('目的','g1',('a',)), 被覆('出力','g1',('g1',)))
        coverage=被覆台帳印(rows)
        base=dict(原文='現在の依頼',計画印='plan',要求被覆印=coverage,目的印='goal',素材印='data')
        seal=要求境界契約印(**base)
        variants=(
            {**base,'原文':'前の依頼'},
            {**base,'計画印':'other-plan'},
