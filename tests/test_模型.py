...
class 模型核試験(unittest.TestCase):
    def test_文脈差が成立差へ到達する(self):
        ...
        japan = 模型核.評価言語状態(言語状態("日本 首都", "自然言語:ja"), candidates)
        france = 模型核.評価言語状態(言語状態("フランス 首都", "自然言語:ja"), candidates)
    ...
    def test_同じ関係を複数文脈へ再利用する(self):
        ...
        first = 模型核.評価言語状態(言語状態("日本 地理", "自然言語:ja"), candidates)
        second = 模型核.評価言語状態(言語状態("日本 都市", "自然言語:ja"), candidates)
    ...
    def test_根拠差がなければ勝手に確定しない(self):
        ...
        self.assertIsNone(結果.最有力候補ID)
        self.assertEqual(結果.候補辞書(), {"A": 0, "B": 0})
    ...
    def test_プログラム言語も明示体系として扱える(self):
        ...
        self.assertEqual(結果.最有力候補ID, "value")

    def test_模型核は適合器外の無言言語体系混在を拒否する(self):
        模型核 = MINIDORA模型核(())
        candidates = (成立候補("A", 言語状態("A", "自然言語:ja")),)
        with self.assertRaisesRegex(ValueError, "候補と言語文脈の言語体系が一致しない"):
            模型核.評価言語状態(言語状態("question", "自然言語:en"), candidates)
    ...
    def test_実行系が模型核入口を持つ(self):
        ...
        body = ミニドラ(模型核_=模型核)
        結果 = body.言語評価("日本", ("東京", "パリ"))
        self.assertEqual(結果.最有力候補ID, "候補1")
        self.assertEqual(結果.候補辞書()["候補1"], 4)
