...
class Runtime射影V15試験(unittest.TestCase):
    ...
    def test_Kは条件scopeを無条件化せず修飾付き関係として保持する(self) -> None:
        projected = HDSKData射影(_合成関係中間表現(conditions=("条件scope=if condition X",)))
        self.assertEqual(len(projected.関係), 1)
