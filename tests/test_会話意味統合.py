...
class 会話統合試験(unittest.TestCase):
    ...
    def test_単位倍率を同じ次元へ揃える(self):
        資料={'A':能力結果(True,'{"電圧":1000,"単位":"mV"}'),'B':能力結果(True,'{"電圧":2,"単位":"V"}')}
        self.assertIn('差（右－左）は1V',self.ok('資料「A」と資料「B」の電圧を比較して',資料).本文)
    def test_指定した表示単位を保持する(self):
        資料={'A':能力結果(True,'{"電圧":1,"単位":"V"}'),'B':能力結果(True,'{"電圧":2,"単位":"V"}')}
        r=self.ok('資料「A」と資料「B」の電圧をmVで比較して',資料)
    ...
    def test_異次元比較は拒否する(self):
        資料=素材();資料['B']=能力結果(True,'{"売上":60,"単位":"V"}')
        r=self.s.応答(Q,資料);self.assertFalse(r.成立);self.assertEqual(len(r.追跡['試行']),1)
    def test_異なる条件を混ぜない(self):
        資料={'A':能力結果(True,'{"売上":75,"単位":"円","条件":"国内"}'),
              'B':能力結果(True,'{"売上":60,"単位":"円","条件":"海外"}')}
        self.assertFalse(self.s.応答(Q,資料).成立)
    def test_片側だけの条件も消さない(self):
        資料=素材();資料['A']=能力結果(True,'{"売上":75,"単位":"円","条件":"国内"}')
        self.assertFalse(self.s.応答(Q,資料).成立)
    def test_未知の備考は短文化で捨てない(self):
        資料=素材();資料['B']=能力結果(True,'{"売上":60,"単位":"円","備考":"暫定値"}')
        self.assertFalse(self.s.応答(Q,資料).成立)
    def test_複合条件を単純条件へ読み替えない(self):
        資料=素材();資料['B']=能力結果(True,'{"売上":60,"単位":"円","条件":{"国内":false}}')
        self.assertFalse(self.s.応答(Q,資料).成立)
    def test_単位欄の真偽値を無視しない(self):
        資料=素材();資料['B']=能力結果(True,'{"売上":60,"単位":false}')
        self.assertFalse(self.s.応答('資料「A」と資料「B」の売上を円で比較して',資料).成立)
    def test_無断の時点差を比較しない(self):
        資料={'A':能力結果(True,'{"売上":75,"単位":"円","年":2025}'),'B':能力結果(True,'{"売上":60,"単位":"円","年":2026}')}
        self.assertEqual(self.s.応答(Q,資料).状態,'確認待ち')
    ...
    def test_複数行を無断で先頭採用しない(self):
        資料=素材();資料['B']=能力結果(True,'年,売上,単位\n2025,60,円\n2026,30,円')
        r=self.s.応答(Q,資料);self.assertEqual(r.状態,'確認待ち')
    ...
    def test_失敗した直下経路だけを再開放する(self):
        資料=素材();資料['B']=能力結果(True,'{"items":[{"売上":60,"単位":"円"}]}')
        r=self.ok(Q,資料);self.assertEqual(len(r.追跡['試行']),2)
    ...
    def test_両方の入れ子も二つの別失敗から修復する(self):
        資料={n:能力結果(True,json.dumps({'箱':[json.loads(v.本文)]},ensure_ascii=False)) for n,v in 素材().items()}
        r=self.ok(Q,資料);self.assertEqual(len(r.追跡['試行']),3)
    def test_異なる階層の同名属性は勝手に選ばない(self):
        資料=素材();資料['B']=能力結果(True,'{"売上":60,"単位":"円","子":{"売上":30,"単位":"円"}}')
        self.assertEqual(self.s.応答(Q,資料).状態,'確認待ち')
    def test_入れ子に複数候補があれば保留する(self):
        資料=素材();資料['B']=能力結果(True,'{"箱":[{"売上":60},{"売上":30}]}')
        self.assertEqual(self.s.応答(Q,資料).状態,'確認待ち')
    def test_JSON数値文字列を数値へ自動変換しない(self):
        資料=素材();資料['B']=能力結果(True,'{"売上":"60","単位":"円"}')
        self.assertFalse(self.s.応答(Q,資料).成立)
    def test_真偽値を整数へ変換しない(self):
        資料=素材();資料['B']=能力結果(True,'{"売上":true,"単位":"円"}')
        self.assertFalse(self.s.応答(Q,資料).成立)
    def test_欠落値をゼロにしない(self):
        資料=素材();資料['B']=能力結果(True,'{"売上":null,"単位":"円"}')
        self.assertFalse(self.s.応答(Q,資料).成立)
    def test_小数は正確な値で差を取る(self):
        self.assertIn('差（右－左）は0.1円',self.ok(Q,素材(0.1,0.2)).本文)
    def test_指数表記は有理数として処理する(self):
        資料=素材();資料['B']=能力結果(True,'{"売上":6e1,"単位":"円"}')
        self.assertIn('-15円',self.ok(Q,資料).本文)
    def test_巨大指数を処理しない(self):
        資料=素材();資料['B']=能力結果(True,'{"売上":6e1000,"単位":"円"}')
        self.assertFalse(self.s.応答(Q,資料).成立)
    def test_重複JSONキーを後勝ちにしない(self):
        資料=素材();資料['B']=能力結果(True,'{"売上":60,"売上":90,"単位":"円"}')
        self.assertFalse(self.s.応答(Q,資料).成立)
    ...
    def test_短い回答でも条件は保持する(self):
        資料={n:能力結果(True,json.dumps({**json.loads(v.本文),'条件':'国内'},ensure_ascii=False)) for n,v in 素材().items()}
        self.assertIn('条件：国内',self.ok(Q+'。短く説明して',資料).本文)
    ...
    def test_回答資料の値とhashを変えても原本再構成で拒否する(self):
    ...
    def test_ユーザー発話を事実採用と混同しない(self):
        ...
        状態=self.s.状態();self.assertIn('事実として未採用',状態['発話'][0]['入力の扱い'])
        self.assertEqual(self.s.統合.採用履歴スナップショット()[1],())
    ...
    def test_未知HDS残差を無視しない(self):
        ...
        bad=replace(ir,残差=(HDS残差('bad','意味_loss','条件','未解釈'),))
    ...
    def test_入力資料の後変更が保存資料へ混入しない(self):
        資料=素材();self.ok(Q,資料);資料['A']=能力結果(True,'{"売上":999}')
