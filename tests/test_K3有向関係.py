...
from minidora.K3_HDSネイティブ import HDSIRネイティブAdapter, _edge_similarity, _意味署名
...
def _question(*, with_relation: bool = False) -> HDSIR:
    relations = (HDS関係("question-context", ("alpha",), ("beta",), "因果"),) if with_relation else ()
    return _ir(
...
def _run_direction_case(*, question_relation: bool = False):
    core = K3相当能力核()
    data_ir = _ir(
        "Alpha causes Beta.",
        (
    ...
    return HDSIRネイティブAdapter(core).実行(_question(with_relation=question_relation), 候補IR=candidates)
...
class HDS関係方向試験(unittest.TestCase):
    def test_同じ語と関係種別でも逆向きは一致扱いしない(self) -> None:
        ...
        evidence = _意味署名(_方向候補("Alpha causes Beta", reverse=False))
    ...
    def test_問い側の共通方向を逆向き候補へ伝染させない(self) -> None:
        result = _run_direction_case(question_relation=True)
