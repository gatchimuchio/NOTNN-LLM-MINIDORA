...
from minidora.監査改善接続 import 監査改善モジュール
...
class 純粋再利用回帰試験(unittest.TestCase):
    def setUp(self):
        self.モジュール = 監査改善モジュール('拡張命題検討')
        self.cache = 純粋結果庫()
        self.wrapper = 純粋再利用能力(self.モジュール, self.cache)
    ...
    def test_実能力の再利用で意味全体が一致し原実行が減る(self):
        ...
        stats = self.cache.統計()['能力別'][self.モジュール.名前]
    ...
    def test_問い変更を同一本文キャッシュとして取り違えない(self):
        ...
        self.assertEqual(self.cache.統計()['能力別'][self.モジュール.名前]['再利用'], 0)
    ...
    def test_未知設定で失敗した結果を再利用しない(self):
        資料 = 文脈(); 資料.補助['合成設定']['未対応'] = True
        for _ in range(2):
            self.assertFalse(self.wrapper.実行(資料).成立)
        self.assertEqual(self.cache.統計()['件数'], 0)
        self.assertEqual(self.cache.統計()['能力別'][self.モジュール.名前]['原実行'], 2)

    def test_明示無効時はキャッシュに保存しない(self):
        store = 純粋結果庫(有効=False); wrapper = 純粋再利用能力(self.モジュール, store)
        self.assertEqual(_結果辞書(wrapper.実行(文脈())), _結果辞書(wrapper.実行(文脈())))
        self.assertEqual(store.統計()['件数'], 0)
        self.assertEqual(store.統計()['能力別'][self.モジュール.名前]['原実行'], 2)

    def test_登録後の版変更はヒットでも拒否する(self):
        self.wrapper.実行(文脈()); self.モジュール.版 = 'changed'
    ...
    def test_文脈と参照の差を鍵から落とさない(self):
        ...
        self.assertEqual(self.cache.統計()['能力別'][self.モジュール.名前]['原実行'], 3)


class 意味射影試験(unittest.TestCase):
    def setUp(self):
        self.session = 統合セッション('射影', 基底能力=改善統合能力群())
        plan = 改善目的を計画('命題', original(), self.session.能力一覧())
        self.reply = self.session.計画実行(plan.計画, plan.資料)
    ...
    def test_実統合器の二回実行も全意味一致で再利用する(self):
        ...
        cache = 純粋結果庫()
        group = tuple(登録能力(純粋再利用能力(r.モジュール, cache)) for r in 改善統合能力群())
        session = 統合セッション('twice', 基底能力=group)
        plan = 改善目的を計画('命題', original(), session.能力一覧())
        one = session.計画実行(plan.計画, plan.資料)
        first = cache.統計()
        two = session.計画実行(plan.計画, plan.資料)
        second = cache.統計()
