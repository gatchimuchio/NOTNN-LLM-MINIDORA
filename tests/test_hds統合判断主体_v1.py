...
def choice_ir(*, required: bool = False) -> HDSIR:
    return HDSIR(
        ...
        認知世界ID="test-world",
        座標=(HDS座標("choice:A", "候補", "猫"), HDS座標("choice:B", "候補", "犬")),
...
class HDS統合判断主体試験(unittest.TestCase):
    def test_参照_計算_COMMITを判断主体が順番に承認する(self):
        ...
        self.assertEqual(tuple(action for action, _ in result.認知世界.作用履歴), ("REFERENCE", "EVALUATE", "COMMIT"))
        self.assertEqual(result.選択.状態, "PROPOSE")
    ...
    def test_候補生成結果だけでは自己COMMITしない(self):
        ...
        self.assertEqual(subject.確定(world).状態, "COMMITTED")
    ...
    def test_必須参照が無ければ計算へ進まずSUSPENDする(self):
        ...
        self.assertEqual(tuple(action for action, _ in result.認知世界.作用履歴), ("SUSPEND",))
