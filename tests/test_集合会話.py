...
def 資料を作る(values=(75,60,95), *, unit='円', nested=False):
    結果={}
    for i,value in enumerate(values):
        資料={'売上':value,'費用':i+1}
        if unit: 資料['単位']=unit
        if nested: 資料={'箱':資料}
        結果[chr(65+i)]=能力結果(True,json.dumps(資料,ensure_ascii=False))
    return 結果
...
class 集合会話試験(unittest.TestCase):
    ...
    def test_九対象を八対象へ切り捨てない(self):
        self.assertFalse(self.s.応答('全資料の売上の合計は？',資料を作る(tuple(range(9)))).成立)
    ...
    def test_計算手順が原成果から導出される(self):
        self.ok(資料=資料を作る());r=self.ok('計算過程も説明して')
        self.assertIn('(75) + (60) + (95)',r.本文);self.assertIn('平均 = 合計 / 3',r.本文)
    def test_資料名の表記をMarkdown列にしない(self):
        資料={'東|部':資料を作る()['A'],'西':資料を作る()['B'],'南':資料を作る()['C']}
        r=self.ok('全資料の売上の合計は？表で',資料)
    ...
    def test_停止後に成果も資料も更新しない(self):
        r=self.s.応答(Q,資料を作る(),停止要求=lambda:True)
        self.assertEqual(r.状態,'中止');self.assertEqual(self.s.状態()['資料版'],{})
    def test_異なる条件の数量を平均しない(self):
        資料=資料を作る();資料['B']=能力結果(True,'{"売上":60,"単位":"円","条件":"別条件"}')
        self.assertFalse(self.s.応答(Q,資料).成立)
    def test_無断で異時点を集約しない(self):
        資料=資料を作る();資料['B']=能力結果(True,'{"売上":60,"単位":"円","年":2025}')
        self.assertFalse(self.s.応答(Q,資料).成立)
    def test_誤ったJSON型を別経路で数値に変えない(self):
        資料=資料を作る();資料['C']=能力結果(True,'{"売上":"95","単位":"円"}')
        r=self.s.応答(Q,資料);self.assertFalse(r.成立);self.assertEqual(len(r.追跡['試行']),1)
    def test_未解釈の注記を平均から捨てない(self):
        資料=資料を作る();資料['C']=能力結果(True,'{"売上":95,"単位":"円","備考":"暫定"}')
        self.assertFalse(self.s.応答(Q,資料).成立)
    def test_名前付きの集合に同じ対象を二回入れない(self):
        self.assertFalse(self.s.応答('資料「A」と資料「B」と資料「A」の売上の合計は？',資料を作る()).成立)
    def test_未知HDS残差を新しい集合でも免除しない(self):
        req=会話を解釈(Q,('A','B','C'));ir=公開HDSコンパイラ().コンパイル(Q)
        bad=replace(ir,残差=(*ir.残差,HDS残差('bad','意味_loss','条件','未解釈')))
    ...
    def test_集合出力の出典削除を検出(self):
        r=self.ok(資料=資料を作る());value=能力結果を復元(r.結果.データ['元結果'][0])
        self.assertFalse(集合記録整合(replace(value,参照=())))
    def test_回答の出典削除を検出(self):
        r=self.ok(資料=資料を作る());self.assertFalse(回答記録整合(replace(r.結果,参照=())))
    def test_回答の根拠欄削除を検出(self):
        r=self.ok(資料=資料を作る());self.assertFalse(回答記録整合(replace(r.結果,根拠=())))
