...
class 集合会話試験(unittest.TestCase):
    ...
    def test_負数を含む合計(self):
        self.assertEqual(self.group(self.ok(資料=資料((-1,-2,0))))['集約']['合計'],'-3')
    def test_小数を浮動小数の丸めで変えない(self):
        self.assertEqual(self.group(self.ok(資料=資料((0.1,0.2,0.3))))['集約']['平均'],'0.2')
    ...
    def test_属性訂正で再計算する(self):
        old=self.ok(資料=資料());new=self.ok('訂正:属性は費用です')
    ...
    def test_資料更新で集合成果は失効(self):
        self.ok(資料=資料());self.s.応答('資料「C」を更新:{"売上":1,"単位":"円"}')
        self.assertFalse(self.s.応答('それを詳しく説明して').成立)
    def test_再表現しても平均の厳密値と条件を保持(self):
        self.ok(資料=資料());r=self.ok('それを表にして')
    ...
    def test_集合出力の出典削除を検出(self):
        r=self.ok(資料=資料());value=能力結果を復元(r.結果.データ['元結果'][0])
        self.assertFalse(集合記録整合(replace(value,参照=())))
    def test_回答の出典削除を検出(self):
        r=self.ok(資料=資料());self.assertFalse(回答記録整合(replace(r.結果,参照=())))
    def test_回答の根拠欄削除を検出(self):
        r=self.ok(資料=資料());self.assertFalse(回答記録整合(replace(r.結果,根拠=())))
