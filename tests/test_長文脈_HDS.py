...
class 長文脈HDS接続試験(unittest.TestCase):
    ...
    def test_長文脈の原文選択から実HDSと抽出へ(self):
        ...
        結果 = self.session.応答('本文から数字を抽出して', {'本文': material})
        self.assertTrue(結果.成立, 結果.理由)
        self.assertEqual(結果.出力[0][1].本文, '731、75、45')
        self.assertEqual(len(selected.省略ID), 100)
        self.assertEqual(結果.出力[0][1].参照, material.参照)
    ...
    def test_実HDS成果の依存元を改訂すると現行参照しない(self):
        selected, material = self.material()
        結果 = self.session.応答('本文を1行で要約して', {'本文': material})
        self.assertTrue(結果.成立, 結果.理由)
        self.a.成果登録(selected, '成果', 結果.出力[0][1])
        ...
        結果 = self.session.応答('本文から数字を抽出して', {'本文': self.a.資料化(current)})
        self.assertTrue(結果.成立, 結果.理由)
        self.assertEqual(結果.出力[0][1].本文, '999')

    def test_過小予算の不成立資料を実行しない(self):
        selected = self.a.選択(文脈選択要求(('原文',), 最大バイト数=1))
        結果 = self.session.応答('本文から数字を抽出して', {'本文': self.a.資料化(selected)})
