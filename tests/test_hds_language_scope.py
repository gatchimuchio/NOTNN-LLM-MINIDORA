...
def _K阻害関係(ir):
    return [relation for relation in HDSKData射影(ir).関係 if str(relation.種別) == "阻害"]
...
class HDS英語RelationScope試験(unittest.TestCase):
    ...
    def test_mayを実体端点と可能scopeへ分離しDataで保持する(self) -> None:
    ...
    def test_mustを必要scopeとしてDataで保持する(self) -> None:
