...
from minidora.製品版.計算 import 計算Module
from minidora.製品版.要約 import 汎用要約Module
...
class Coreブラッシュアップ試験(unittest.TestCase):
    ...
    def test_Core保留を値の有無だけで合格へ昇格しない(self):
        core = 状態付きCore(実行状態.保留, "部分結果", ("実行入力未確定",))
        app = 製品ミニドラ(基礎ミニドラ=core, 監査改善=False)
        result = app.応答("自由意志について説明して", セッションID="core-hold")
        self.assertEqual(result.経路, "基礎Core")
        self.assertEqual(result.状態, "保留")
        self.assertNotIn("部分結果", result.本文)
        self.assertEqual(core.応答呼出, 0)

    def test_Core失敗を公開応答へフォールバックして合格化しない(self):
        core = 状態付きCore(実行状態.失敗, "部分結果", ("実行失敗",))
        app = 製品ミニドラ(基礎ミニドラ=core, 監査改善=False)
        result = app.応答("自由意志について説明して", セッションID="core-fail")
    ...
    def test_要約の選択と実行は同じ明示対象を使う(self):
        ...
        self.assertGreater(ability.判定(context), 0)
        result = ability.実行(context)
        self.assertTrue(result.成立)
        self.assertIn("新しい本文", result.本文)
        self.assertNotIn("古い本文", result.本文)

    def test_明示要約対象が空なら直前応答で代用しない(self):
        ability = 要約能力(汎用要約Module())
        context = 能力文脈("次の文章を要約してください。", "summary-empty", "古い本文です。", (), (), {})
        self.assertGreater(ability.判定(context), 0)
        result = ability.実行(context)
        self.assertFalse(result.成立)
        self.assertIn("本文がない", result.保留理由)

    def test_標準Runtimeは公開HDS_Compilerを必ず接続する(self):
        body = 標準ミニドラ()
