...
class 製品接続試験(unittest.TestCase):
    ...
    def send(self, text, session='default'):
        result = self.product.応答(text, セッションID=session)
        self.assertEqual(result.状態, '合格', result.本文)
        return result

    def test_既定製品から有限命題へ到達(self):
        self.send(登録)
        result = self.send(問い)
        self.assertEqual(result.経路, '汎用会話')
        self.assertIn('「支持」', result.本文)
        trace = result.メタデータ['汎用追跡']
        self.assertEqual(trace['HDS原文'], 問い)
        self.assertTrue(trace['監査改善']['合成監査整合'])
        self.assertEqual(trace['監査改善']['要求']['問い'], 'Q')
        self.assertTrue(result.参照)
    ...
    def test_既定製品から有限仮説と確認継続へ到達(self):
        ...
        result = self.send('観測を「B」にして')
        self.assertIn('Aを仮定', result.本文)
        self.assertIn('事実ではありません', result.本文)

    def test_既定製品から有限介入へ到達(self):
        self.send('介入資料「因」を登録：\n外生：U=真\n構造：A=U\n構造：B=A')
        result = self.send('資料「因」で「B=偽」に介入した結果を比較して、短く説明して')
        self.assertIn('Bは真から偽', result.本文)
        self.assertIn('モデル内', result.本文)

    def test_従来の計算経路を保持(self):
        result = self.send('2+3')
        self.assertNotEqual(result.経路, '汎用会話')
        self.assertIn('5', result.本文)
    ...
    def test_別話題の短縮を旧命題成果へ結び付けない(self):
        ...
        result = self.product.応答('もう少し短く説明して')
        self.assertNotEqual(result.経路, '汎用会話')
        self.assertNotIn('「支持」', result.本文)
        self.assertIn('「支持」', self.send(問い).本文)

    def test_保留した要求をCoreの自由回答へ落とさない(self):
        class 通過禁止:
            def 応答(self, text):
                raise AssertionError('改善会話の不成立からCoreへ透過してはいけない')
        self.product = self.factory(基礎ミニドラ=通過禁止())
        self.send(登録)
        result = self.product.応答('資料「例」から「Q」を判断して、根拠を捨てて')
    ...
    def test_資料訂正と再説明失効を製品から実行(self):
        ...
        result = self.product.応答('詳しく説明して')
        self.assertEqual(result.状態, '保留')
        self.assertIn('失効', result.本文)
        self.assertIn('「未確定」', self.send('もう一度').本文)

    def test_製品初期化後に旧資料を使わない(self):
        self.send(登録); self.send(問い)
        self.send('/初期化')
        result = self.product.応答(問い)
        self.assertNotIn('「支持」', result.本文)
    ...
    def test_述語別名を製品の現在目的へ適用する(self):
        ...
        result = self.send('述語別名を「ネコ/1=猫」にして')
        self.assertIn('「支持」', result.本文)
        self.assertIn('明示定義', result.本文)
