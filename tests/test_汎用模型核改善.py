...
class 汎用模型核改善試験(unittest.TestCase):
    def test_同一資料の出典ID違いは票を増幅しない(self) -> None:
        def 評価(refs):
    ...
    def test_異なる資料は別資料として保持する(self) -> None:
        結果 = 標準能力模型核().評価言語状態(
            言語状態("which"),
            (成立候補("A", 言語状態("alpha")), 成立候補("B", 言語状態("beta"))),
            参照状態=(言語状態("alpha first", 識別子="r-alpha"), 言語状態("beta first", 識別子="r-beta-1"), 言語状態("beta second", 識別子="r-beta-2")),
        )
        self.assertEqual(len(結果.文脈.参照状態), 3)
        self.assertEqual(結果.参照最有力候補ID, "B")
    ...
    def test_再作用は現在首位と最強変化候補を比較する(self) -> None:
        ...
        class 境界再作用:
            ...
            def 評価群(self, 文脈, 候補群):
                out = {}
                for cid, 状態 in 候補群:
                    if "challenger" in 状態.意味語集合: out[cid] = 関係寄与(self.名称, 2, ("primary",))
                    elif "decoy" in 状態.意味語集合: out[cid] = 関係寄与(self.名称, 2, ("primary",))
                return out
            def 再評価群(self, 文脈, 候補群, round_index):
                字句 = {cid: 状態.意味語集合 for cid, 状態 in 候補群}
                leader = next((cid for cid, vals in 字句.items() if "leader" in vals), None)
                challenger = next((cid for cid, vals in 字句.items() if "challenger" in vals), None)
                if leader is None or challenger is None: return {}
                return {challenger: 関係寄与(f"境界再作用:{round_index}", 3, ("leader-vs-challenger",))}
        模型核 = MINIDORA能力状態差模型核((初期差(),), 能力作用群=(境界再作用(),), 最大再作用回数=1)
        結果 = 模型核.評価言語状態(
            言語状態("question"),
            (成立候補("A", 言語状態("leader")), 成立候補("B", 言語状態("challenger")), 成立候補("C", 言語状態("decoy"))),
        )
        self.assertEqual(結果.最有力候補ID, "B")
        self.assertEqual(結果.統計.検査点再活性数, 1)
