...
class Core状態差再作用補強試験(unittest.TestCase):
    def test_得点不変でも寄与構造差が続けば再作用を継続する(self) -> None:
        ...
        self.assertEqual(result.統計.再作用回数, 3)
        self.assertGreaterEqual(result.統計.寄与状態再利用数, 6)
        self.assertTrue(any(cp.段階 == "RECONCILE_3" for cp in result.checkpoint))
