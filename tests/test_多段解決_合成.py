...
from minidora.多段解決接続 import 多段解決モジュール, 解決補助能力群
from minidora.能力合成 import 能力合成器, 合成工程, 合成計画, 素材参照, _結果辞書
from minidora.能力合成_局所接続 import 局所能力群
from minidora.証拠統合接続 import 証拠統合モジュール
from minidora.関係制約 import 関係式, 関係記録整合
from minidora.関係制約接続 import 数値関係化モジュール, 関係制約モジュール, 関係判定採用モジュール
...
def 関係問題を用意(上限="100", 条件=None):
    ...
    資料 = {"A": 情報源("a", "装置Aの電圧は120 V以上です。"),
            "B": 情報源("b", (f'条件「{条件}」では、' if 条件 else "") + f"装置Bの電圧は{上限} V以下です。"),
            "指示": 能力結果(True, "指定した処理"),
    ...
    engine = 多段解決器((*解決補助能力群(), 証拠統合モジュール().登録(), 数値関係化モジュール().登録(),
                        関係制約モジュール().登録(), 関係判定採用モジュール().登録()), 純粋作用確認=True)
    return engine, 多段問題(("比較確定",), goals, methods), 資料
...
class 既存能力多段接続試験(unittest.TestCase):
    ...
    def test_参照来歴を後戻りで失わない(self):
        p, 資料 = 例題()
        r = 文書実行器().実行(p, 資料)
        self.assertEqual(r.出力[0][1].参照, 資料["原資料"].参照)
    ...
    def test_未解釈の留保を経路探索で消さない(self):
        engine, p, 資料 = 関係問題を用意()
        ref = 資料["A"].参照[0]
        資料["A"] = replace(資料["A"], 参照=(replace(ref, 本文=ref.本文 + "ただし仮定である。"),))
        r = engine.実行(p, 資料)
        self.assertEqual(r.状態, "保留")
        self.assertEqual(r.出力, ())

    def test_既存合成器から多段解決モジュールを呼べる(self):
        p, 資料 = 例題()
        モジュール = 多段解決モジュール((*局所能力群(), *解決補助能力群()), 純粋作用確認=True)
        payload = 能力結果(True, "", データ={"問題": asdict(p), '初期資料': {k: _結果辞書(v) for k, v in 資料.items()}})
        outer = 合成計画((合成工程("実行", ("多段解決",), "指示", (素材参照("入力", "問題"),)),), ("実行",))
        結果 = 能力合成器((モジュール.登録(),)).実行(outer, {"問題": payload, "指示": 能力結果(True, "目的解決")})
    ...
    def test_普通の会話文を実行可能問題に見せない(self):
        モジュール = 多段解決モジュール(解決補助能力群(), 純粋作用確認=True)
        c = 能力文脈("全部完成させて", "s")
    ...
    def test_独立CLIの後戻り代替除去と数値摂動(self):
        for args in ([], ["--代替なし"], ["--値", "731"]):
            with self.subTest(args=args):
                結果 = subprocess.run([sys.executable, str(ROOT / "tools/多段解決デモ.py"), *args],
                                        capture_output=True, encoding="utf-8", timeout=15)
                self.assertEqual(結果.returncode, 0, 結果.stderr)
                資料 = json.loads(結果.stdout)
                self.assertEqual(資料["状態"], "保留" if "--代替なし" in args else "合格")
