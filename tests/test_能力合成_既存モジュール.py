'既存の実装を呼ぶ接続試験。LLM、擬似模型核、Web応答による代替はない。'
...
class 既存モジュール合成試験(unittest.TestCase):
    def setUp(self):
        self.runner = 能力合成器(局所能力群())
        self.plan, self.資料 = 例題()

    def test_要約の後状態が抽出結果へ因果的に到達(self):
        baseline = 情報抽出モジュール().実行("数字", self.資料["入力資料"].本文)
        結果 = self.runner.実行(self.plan, self.資料)
        self.assertTrue(結果.成立, 結果.理由)
        self.assertEqual(baseline.本文, "120、75、45")
        self.assertEqual(結果.出力[0][1].本文, "120")
        self.assertEqual(dict(結果.中間結果)["要約結果"].本文, "売上は120です。")
    ...
    def test_実資料の来歴が終端まで残る(self):
        ref = 参照資料("入力原典", "提供文", "利用者提供", 本文=self.資料["入力資料"].本文)
        self.資料["入力資料"] = replace(self.資料["入力資料"], 参照=(ref,))
    ...
    def test_要約設定を黙って丸めない(self):
        for v in (0, 9, "3", True):
            with self.subTest(v=v):
                self.資料["要約設定"] = 能力結果(True, "", データ={"行数": v})
                r = self.runner.実行(self.plan, self.資料)
    ...
    def test_未対応設定を無視しない(self):
        self.資料["要約設定"] = 能力結果(True, "", データ={"行数": 1, "翻訳": "英語"})
        r = self.runner.実行(self.plan, self.資料)
        self.assertFalse(r.成立)
    ...
    def test_URL抽出を既存実装へ接続(self):
        ...
        self.資料["入力資料"] = 能力結果(True, "参照先 https://example.test/item と本文")
        self.資料["抽出設定"] = 能力結果(True, "", データ={"種別": "URL"})
        r = self.runner.実行(p, self.資料)
        self.assertTrue(r.成立)
