...
def _result(*contributions: 関係寄与, reverse: bool = False) -> 模型結果:
    ...
    context = 文脈付き言語状態(internal, 条件=("選択意図=反転",) if reverse else ())
    return 模型結果(
        context,
...
class 参照確定品質試験(unittest.TestCase):
    ...
    def test_信頼値をGateの最終採否に使わない(self):
        q = 参照確定品質判定(
            _result(関係寄与("参照関係寄与", 2, ("参照:r1:2",))),
            参照識別子=("r1",), 参照信頼=(0.0,),
        )
    ...
    def test_作用差分連結は強い構造証拠として閉包する(self):
        q = 参照確定品質判定(
            _result(関係寄与("候補共同参照:状態差連結", 1, ("状態差連結:0:S2",))),
            参照識別子=("r1",), 参照信頼=(1.0,),
        )
        self.assertTrue(q.閉包)
        self.assertEqual(q.理由, "STRUCTURAL_EVIDENCE_CLOSED")
