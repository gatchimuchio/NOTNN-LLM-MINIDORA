...
GLM-5.3系D4構文化との比較は次を入口にする。

- `../K3_GLM_作用比較索引_v1.md` — K3 / GLM / MINIDORAを同じ作用座標で横並び比較。
- `../GLM_5_3_能力成立作用構文化_D4_v1/02_K3との差分.md` — GLM側から見たK3との差分。
- `../../src/minidora/構文化由来.py` — K3 / GLM双方の機械可読還元台帳。

K3の本成果は履歴正本として変更せず、GLM由来の追加作用は別構文化・別設計契約へ分離する。
...
本成果はMINIDORA実装仕様ではない。実装候補は `03_MINIDORA射影候補.md` に隔離し、既存の模型核・確定K・HDS 構文化器へ自動昇格しない。
